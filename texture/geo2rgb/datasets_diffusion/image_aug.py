...
if __name__ == "__main__":
    image_path = "/apdcephfs_cq8/share_2909871/Assets/objaverse/render/multiview_diffusion/6view_tilt_20241016/debug/render_data50/pmx/TowerFantacy_FeiSe_FeiSe_denoise_manifold_full/render_512_Valour/color/cam-0000.png"
    image = cv2.imread(image_path, flags=cv2.IMREAD_UNCHANGED)
    # shadow_img = add_shadow(image)
    # cv2.imwrite("shadow_img.png", shadow_img)

    # shadow_img2 = add_shadow_mask(image)
    # cv2.imwrite("shadow_img2.png", shadow_img2)

    # image1 = sketch_aug(image)
    # cv2.imwrite("sketch_aug.png", image1)

    # image2 = gray_aug(image)
    # cv2.imwrite("gray_aug.png", image2)

    # image3 = invert_aug(image)
    # cv2.imwrite("invert_aug.png", image3)

    # result_img1 = np.uint8(sobel(image, threshold=100))
    # cv2.imwrite("sobel.png", result_img1)

    # result_img2 = np.uint8(canny(image, sigma=1.3, kernel_size=(7, 7), ratiomin=0.08, ratiomax=0.5))
    # cv2.imwrite("canny.png", result_img2)

    # result_img3 = np.uint8(laplacian(image, operator=1, threshold=225))
    # cv2.imwrite("laplacian.png", result_img3)

    # result_img4 = np.uint8(LoG(image, sigma=1.3, kernel_size=(7, 7), threshold=100))
    # cv2.imwrite("LoG.png", result_img4)

    # result_img5 = np.uint8(DoG(image, k=1.6, sigma=1, kernel_size=(5, 5), threshold=-1))
    # cv2.imwrite("DoG.png", result_img5)

    result_img6 = np.uint8(XDoG(image, p=45, k=1.6, sigma=1, kernel_size=(5, 5), threshold=100, phi=0.025))
    cv2.imwrite("XDoG.png", result_img6)