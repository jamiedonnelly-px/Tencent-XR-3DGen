...
def to_normal_image_mask(normal_path, mask, resolution):
    ...
    # print(normal.dtype)
    # breakpoint()
    # cv2.imwrite("test.png", normal)
    # normal = cv2.resize(normal, (int(resolution), int(resolution)), interpolation=cv2.INTER_NEAREST)
    normal = cv2.resize(normal, (int(resolution), int(resolution)), interpolation=cv2.INTER_CUBIC)
    # cv2.imwrite("xyz_tmp.png", normal)
    # print("xyz_reading !!!")
    # breakpoint()
    ...
    normal[0,:,:][mask==0] = 29882.353
    normal[1,:,:][mask==0] = 29882.353
    normal[2,:,:][mask==0] = 29882.353

    # normal[0,:,:][mask==0] = 127.0
    # normal[1,:,:][mask==0] = 127.0
    # normal[2,:,:][mask==0] = 127.0

    # 将图像转换为 PyTorch 张量
    # normal_tensor = torch.from_numpy(normal) / 255.0
    normal_tensor = torch.from_numpy(normal) / 60000.0
    # normal_tensor = torch.from_numpy(normal) / 255.0
...
class ObjaverseDatasetV3_2_090180270_controlnet(Dataset):
    def __init__(self,
        configs,
        data_type = "train",
    ...
    def get_z123_item(self, index, only_first_prob=0.3):
        ...
        xyz_dir = os.path.join(image_dir, "xyz")
        xyz_dir_train = os.path.join(image_dir_train, "xyz")
        target_xyz_dir = xyz_dir
        ...
        for image_idx in image_sub_idx_cond_list:
            ...
            cond_image, _ = to_rgb_image(cond_image_alpha_rescale, self.bg_color_num, edge_aug_threshold=0.4)
        ...
        for image_idx in image_sub_idx_target_list:
            ...
            xyz_path = os.path.join(target_xyz_dir, f"cam-{str(image_idx).zfill(4)}.png")
            xyz_train_path = os.path.join(xyz_dir_train, f"cam-{str(image_idx).zfill(4)}.png")
            xyz_path = get_train_path(xyz_path, xyz_train_path)
            ...
            xyzs_path_list.append(xyz_path)
            try:
                target_image = Image.open(img_path)
                bright_scale = get_brightness_scale(cond_image0, target_image)
                target_image, mask = to_rgb_image(target_image, self.bg_color_num, bright_scale=bright_scale)
                target_image = self.train_transforms(target_image)
                images_out.append(target_image.squeeze())

                # target_xyz = Image.open(xyz_path)
                target_xyz = to_normal_image_mask(xyz_path, mask, self.img_out_resolution)
                xyzs_out.append(target_xyz.squeeze())
            except:
                return self.get_z123_item(np.random.randint(0, self.__len__() - 1))
        ...
        xyzs_out = torch.stack(xyzs_out, dim=0)
...
if __name__ == "__main__":
    ...
    train_dataset = ObjaverseDatasetV3_2_090180270_controlnet(configs, data_type="train", load_from_cache_last=True)
    ...
    for data in train_dataloader:
        ...
        xyzs_out = make_grid(xyzs_out[0], nrow=2, padding=0)
        save_image(xyzs_out, "z123_xyzs_out.png")
