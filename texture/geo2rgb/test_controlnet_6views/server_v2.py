...
from pipeline.pipeline_zero123plus_v2 import Zero123PlusPipeline
...
class Zero123plusServer:
    ...
    def inference(self, image_origin_path, image_path, image_mask_path, wh_ratio=0.7, cfg_scale=3.0, step_num=75, job_id_dir="", **kwargs):
        if not os.path.exists(image_origin_path):
            logging.error(f"file not exists: {image_origin_path}")
            return None
        try:
            image, image_mask = process_image_path(image_origin_path, bg_color=255, wh_ratio=wh_ratio)

            ### save processed image
            image.save(image_path)
            image_mask.save(image_mask_path)

            ### condition image size 512
    ...
    def parse_and_run(self, msg):
        ...
        image_preprocessed_mask_path = os.path.join(job_id_dir, f"preprocess_z123_mask.png")
        ...
        inference_param = {
                            ...
                            "image_origin_path": call_data["in_image_path"],
                            "image_path": image_preprocessed_path,
                            "image_mask_path": image_preprocessed_mask_path,
                            "wh_ratio": call_data.get("wh_ratio", 0.8),
                            "cfg_scale": call_data.get("cfg_scale", 3.0),
                            "step_num": call_data.get("step_num", 75),
                            "result_rmbg": self.result_rmbg,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="zero123plus_v22")
    ...
    print(f"cfg_json: {args.cfg_json}")
    print(f"model_name: {args.model_name}")
    print(f"result_rmbg: {args.result_rmbg}")
