...
def process_image(image, bg_color, wh_ratio=0.8, save_samout=False):
    ...
    image_processed_rgba = image_preprocess(image_sam, save_samout=save_samout, lower_contrast=False, rescale=True, bg_color=bg_color, wh_ratio=wh_ratio)
...
def preprocess_image(image, bg_color=255, wh_ratio=0.8, save_samout=False):
    if isinstance(image, str):
        image = Image.open(image)
    ...
    rgba = process_image(image, bg_color, wh_ratio, save_samout=save_samout)
...
def process_image_path(image_path, bg_color=255, wh_ratio=0.8, save_samout=False):
    return preprocess_image(image_path, bg_color, wh_ratio, save_samout=save_samout)
