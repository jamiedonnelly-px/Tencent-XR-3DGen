...
from pytorch3d.renderer import TexturesUV as TexturesUV_structures

import sys
# sys.path.append(os.path.abspath("/aigc_cfs/xibinsong/code/zero123plus_control/zero123plus/"))
# sys.path.append(os.path.abspath("/aigc_cfs/xibinsong/code/zero123plus_control/zero123plus/utils/"))
...
from geom_renderer import rotate_mesh_x_axis, make_pers_cameras, make_ortho_cameras, get_conditioning_images
...
class DiffRender():
    ...
    # def load_mesh(self, mesh_path, scale_factor=1.0, transformation=None, use_blender_coord=False, auto_center=True, obj_gen_type="lrm"):
    def load_mesh(self, mesh_path, scale_factor=1.0, transformation=None, use_blender_coord=False, auto_center=True):
        ...
        if mesh.textures is None:
            ...
            print("the obj file is not with texture images, need to convert to corresponding obj with textures !!!")
            # breakpoint()
            # output_mesh_path = "/aigc_cfs/xibinsong/code/zero123plus_control/zero123plus/log_obj_files"
            output_mesh_path = "/aigc_cfs/xibinsong/code/zero123plus_control/log_obj_files"
            _, mesh_name = os.path.split(mesh_path)
            save_obj_name = mesh_name + str(time.time()) + ".obj"
            save_obj_name = os.path.join(output_mesh_path, save_obj_name)
            # print(save_obj_name)
            # breakpoint()
            ...
            mesh = load_objs_as_meshes([save_obj_name], device=self.device)
        ...
        # elif obj_gen_type == "mv2mesh":
        #     # Define rotation in terms of euler angles
        #     rotation = torch.Tensor([-1* torch.pi / 2., 0., -1 * torch.pi / 2.]).cuda()
        #     # rotation = torch.Tensor([0., torch.pi / 2., 0.]).cuda()
        #     rotation_matrix = euler_angles_to_matrix(rotation, "XYZ")
        #     rotated_verts_list = [torch.mm(vert, rotation_matrix.T) for vert in mesh.verts_list()]
        #     # Update mesh
        #     # mesh = Meshes(verts=rotated_verts_list, faces=mesh.faces_list())
        ...
        ########## activate the following part if used for gradio code #######

        # # Define rotation in terms of euler angles
        # # rotation = torch.Tensor([-1* torch.pi / 2., 0., -1 * torch.pi / 2.]).cuda()
        # rotation = torch.Tensor([torch.pi / 2., 0., ]).cuda()
        # rotation_matrix = euler_angles_to_matrix(rotation, "XYZ")
        # rotated_verts_list = [torch.mm(vert, rotation_matrix.T) for vert in mesh.verts_list()]
        # # Update mesh
        # ### if use Meshes update mesh, texture information will ignored, and uv information will missing ###
        # # mesh = Meshes(verts=rotated_verts_list, faces=mesh.faces_list())
        # mesh._verts_list = rotated_verts_list
        ...
        # Define rotation in terms of euler angles
        # rotation = torch.Tensor([-1* torch.pi / 2., 0., -1 * torch.pi / 2.]).cuda()
        rotation = torch.Tensor([0., torch.pi / 6., 0.]).cuda()
        rotation_matrix = euler_angles_to_matrix(rotation, "XYZ")
        rotated_verts_list = [torch.mm(vert, rotation_matrix.T) for vert in mesh.verts_list()]
        # Update mesh
        ### if use Meshes update mesh, texture information will ignored, and uv information will missing ###
    ...
    def load_mesh_with_rotate(self, mesh_path, rotate_angle=30, scale_factor=1.0, transformation=None, use_blender_coord=False, auto_center=True):
        # print("here !!!!")
        # breakpoint()
        mesh = load_objs_as_meshes([mesh_path], device=self.device)
        # -> in [-scale_factor, scale_factor] cube
        ...
        # # # Define rotation in terms of euler angles
        # # # rotation = torch.Tensor([-1* torch.pi / 2., 0., -1 * torch.pi / 2.]).cuda()
        rotation = torch.Tensor([0., torch.pi / (180/rotate_angle), 0.]).cuda()
        rotation_matrix = euler_angles_to_matrix(rotation, "XYZ")
        rotated_verts_list = [torch.mm(vert, rotation_matrix.T) for vert in mesh.verts_list()]
        # Update mesh
        ### if use Meshes update mesh, texture information will ignored, and uv information will missing ###
...
def test(in_mesh_path, image_all, out_dir, render_size=256, scale_factor=1.0):
    ...
    cameras = diff_render.make_ortho_cameras(scale_xyz=1.0)
...
if __name__ == "__main__":
    ...
    test(in_mesh_path, image_all, out_dir, render_size=render_size, scale_factor=scale_factor)