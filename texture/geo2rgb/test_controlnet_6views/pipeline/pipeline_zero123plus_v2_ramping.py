...
# sys.path.append("/aigc_cfs/xibinsong/code/z123_controlnet/diffusers_triplane/src")
...
sys.path.append("/aigc_cfs/xibinsong/code/zero123plus_control/zero123plus")
# from src.diffusers.models.unet_2d_condition import UNet2DConditionModel
# from src.diffusers.models.zero123plus_unet import UNet2DConditionModelRamping
# from .src.diffusers.model_z123.zero123plus_unet import UNet2DConditionModelRamping
...
class RefOnlyNoisedUNet(torch.nn.Module):
    def __init__(
        self, unet: UNet2DConditionModelRamping, train_sched: DDPMScheduler, val_sched: EulerAncestralDiscreteScheduler
    ...
    def forward_cond(
        ...
        # is_controlnet = self.unet.get_is_controlnet()
        ...
        **kwargs,
        ...
        if return_cond_pred:
            ...
            cond_pred = self.forward_cond(
                sample, timestep, encoder_hidden_states, class_labels, ref_dict, is_cfg_guidance, **kwargs
            )
            return cond_pred
        else:
            # print("here 2222 !!!!")
            # print(noisy_cond_lat.shape)
            noisy_cond_lat =  noisy_cond_lat.squeeze(0)
            # breakpoint()
            # down_block_additional_residuals = None
            # mid_block_additional_residual = None
            cond_pred = self.forward_cond(
                noisy_cond_lat, timestep, encoder_hidden_states, class_labels, ref_dict, is_cfg_guidance, **kwargs
            )
            ...
            # is_controlnet = self.unet.get_is_controlnet()
            # print("unet_is_controlnet: ", is_controlnet)
            # breakpoint()

            # if down_block_res_samples is None:
            #     print("unet down_block_res_samples is None")
            # else:
            #     print("unet down_block_res_samples is not None")

            # if mid_block_res_sample is None:
            #     print("unet mid_block_res_sample is None")
            # else:
            #     print("unet mid_block_res_sample is not None")

            # breakpoint()

            # self.set_controlnet_info(down_block_res_samples, mid_block_res_sample, weight_dtype)
