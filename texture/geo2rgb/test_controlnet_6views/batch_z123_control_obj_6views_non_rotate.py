...
save_path = "../results/6views_14000_non_rotate_front_" + str(infer_steps) + "_inpaint_1e_5_guidance_scale_" + str(guidance_scale) + "_conditioning_scale_" + str(conditioning_scale)  
...
for folder in files_path:
    ...
    img_name = os.path.join(img_path, "0.png")
    # print(img_name)
    # breakpoint()
    ...
    if os.path.exists(img_name):
        ...
        normal, xyz, xyz_black = render_obj_depth_normal(obj_path=obj_path,out_dir=save_path_each, render_size=img_res, use_ortho=True)
        # breakpoint()
        ...
        result = pipeline(cond, depth_image=depth, controlnet=controlnet, guidance_scale=guidance_scale, conditioning_scale=conditioning_scale, num_inference_steps=infer_steps, width=img_res*2, height=img_res*3).images[0]
