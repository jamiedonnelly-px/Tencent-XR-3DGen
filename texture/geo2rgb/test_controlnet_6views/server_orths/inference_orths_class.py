...
def img_seg(img_path, seg_model, img_size=512, padding_scale = 0.9):
    ...
    # img = repadding_rgba_image(img, rescale=True, ratio=padding_scale, bg_color=255)
    img = repadding_rgba_image(img, rescale=True, ratio=padding_scale, bg_color=255)
...
def img_seg_mesh_texturing(img_path, seg_model, img_size=512):
    # device = torch.device(f"cuda:{'0'}" if torch.cuda.is_available() else "cpu")
    # rmbg = RMBG(seg_model, device)
    # img_path = "/aigc_cfs_gdp/sz/result/pipe_test/639c3cee-085f-4e2d-acfa-fce1150d36b6/mesh2image.png"
    ...
    # img = repadding_rgba_image(img, rescale=True, ratio=0.9, bg_color=255)
    # img = repadding_rgba_image(img, rescale=True, ratio=padding_scale, bg_color=255)
    img = repadding_rgba_image(img, rescale=False, ratio=0.9, bg_color=255)
    img = Image.fromarray(img)
...
def run_xyz2rgb(
    ...
    #     # "/aigc_cfs/xibinsong/models/zero123plus_v24_4views", 
    #     "/aigc_cfs_gdp/xibin/z123_control/models/single_view_models/zero123plus_v24_4views",
    #     # custom_pipeline="/aigc_cfs/xibinsong/code/zero123plus_control/zero123plus_gray/pipeline_online",
    #     custom_pipeline="/aigc_cfs_gdp/xibin/z123_control/code/z123plus_controlnet_gdp/pipeline_online",
    ...
    if in_data_type == 't2i+z123+meshgen':
        cond = Image.open(img_path)
    elif in_data_type == '3viewsgen':
        cond = torch.from_numpy(np.load(img_path)[0])
        cond = cond.permute(1, 2, 0)
        print("cond: ", cond.shape)
        print("cond: ", torch.max(cond))
        print("cond: ", torch.min(cond))
        cond = Image.fromarray(cond.numpy())
    elif in_data_type == 'mv-2-8views+t2i':
        # cond = img_seg(img_path, img_size=512)
        cond = img_seg(img_path, seg_model, img_size=512)
    elif in_data_type == "mesh_texgen":
        cond = img_seg(img_path, seg_model, img_size=512)
    elif in_data_type == "model_texturing":
        cond = img_seg_mesh_texturing(img_path, seg_model, img_size=512)
    else:
        print("in_data_type: ", in_data_type)
        print("invalid in_data_type !!!")
        raise ValueError("invalid in_data_type !!! in_data_type must be one of [t2i+z123+meshgen, 3viewsgen, mv-2-8views+t2i, mesh_texgen, model_texturing]")


    # extension_path = Path(img_path)
    # extension = extension_path.suffix
    ...
    #     cond = Image.fromarray(cond.numpy())
    # else:
    #     print("cond_img_path: ", img_path)
    #     print("invalid condition image extension !!!")
    #     raise ValueError("invalid condition image extension !!!")
    
    # normal, xyz = render_obj_depth_normal(obj_path=obj_path,out_dir=out_dir, render_size=512, use_ortho=True)
    xyz = render_obj_depth_normal(obj_path=obj_path,out_dir=out_dir, render_size=512, use_ortho=True)
    # breakpoint()
    ...
    result = pipeline(cond, depth_image=depth, controlnet=controlnet, guidance_scale=i, conditioning_scale=conditioning_scale, num_inference_steps=inference_steps, width=1024, height=1024).images[0]
    # result.show()
    # result.save(out_dir + "/tmp_" + str(i) +".png")

    depth = np.array(depth)
    result = np.array(result)
    # result[depth==0] = 255.0

    # result = post_process_image(result, depth)
    ...
    # print(depth[:512, :512, :].shape)
    # print(depth[:512, :512, :].unsqueeze(0).shape)
    depth = torch.cat((depth[:512, :512, :].unsqueeze(0), depth[:512, 512:, :].unsqueeze(0), depth[512:, :512, :].unsqueeze(0), depth[512:, 512:, :].unsqueeze(0)), dim=0)
    out = torch.cat((result[:512, :512, :].unsqueeze(0), result[:512, 512:, :].unsqueeze(0), result[512:, :512, :].unsqueeze(0), result[512:, 512:, :].unsqueeze(0)), dim=0)
    # print("depth: ", depth.shape)
    # print("depth: ", torch.max(depth))
    # print("depth: ", torch.min(depth))
    depths = depth.permute(0, 3, 1, 2)
    out = out.permute(0, 3, 1, 2)

    # print("ref_img: ", ref_img.shape)
    # print("depth: ", (depths.clip(0,1) * 255).permute(2,0,3,1).reshape(512,4*512,3).shape)
    ...
    if vis_dir is not None:
        ...
        in_img = torch.cat([ref_img, (depths.clip(0,255)).permute(2,0,3,1).reshape(512,4*512,3)], dim=1)
        ...
        cv2.imwrite(os.path.join(vis_dir, "out.png"), out.permute(2,0,3,1).reshape(512,-1,3).numpy()[...,::-1])
...
#     device = "cuda"
#     seed = 22
#     validation_pipeline = MMDiffusionDepthToRgbImagePipeline.from_pretrained("/aigc_cfs/xibinsong/code/MMD_NPU_code/MMD_NPU_depth_2_rgb/MMD_NPU/examples/mmd/xyz_2_rgb_512_4views/xyz_512_4views/pipeline-10000", num_views=4, num_tasks=1, 
#         torch_dtype=torch.float16)
    
#     validation_pipeline.to(device)
#     validation_pipeline.enable_xformers_memory_efficient_attention()

#     dino_image_processor = ViTImageProcessor.from_pretrained('/aigc_cfs/model/dino-vitb16')
#     dino_model = ViTModel.from_pretrained('/aigc_cfs/model/dino-vitb16')

#     vit_model_name = "/aigc_cfs/model/vit-base-patch16-224"
...
#     # run_d2rgb(validation_pipeline, 
#     #           "/aigc_cfs_gdp/sz/result/pipe_test/31005051-8902-475b-ac6d-d20ac4360bc9/obj_dir/obj_mesh_mesh.obj", 
#     #           "/aigc_cfs_2/zacheng/MMD_NPU_dmmd/tdmq/server/pose.npz", 
#     #           "/aigc_cfs_gdp/sz/result/pipe_test/31005051-8902-475b-ac6d-d20ac4360bc9/mario.npy", 
#     #           "/aigc_cfs_2/zacheng/MMD_NPU_dmmd/tmp_results/out", 
#     #           "/aigc_cfs_2/zacheng/MMD_NPU_dmmd/tmp_results/vis", 
#     #           seed=0, cfg=3.0)
    
#     run_xyz2rgb(validation_pipeline,
...
#             "/aigc_cfs_gdp/sz/result/pipe_test/31005051-8902-475b-ac6d-d20ac4360bc9/obj_dir/obj_mesh_mesh.obj", 
#             "/aigc_cfs_gdp/sz/result/pipe_test/31005051-8902-475b-ac6d-d20ac4360bc9/mario.npy",  
#             "/aigc_cfs/xibinsong/code/MMD_NPU_code/MMD_NPU_depth_2_rgb/MMD_NPU/test_res/out", 
#             "/aigc_cfs/xibinsong/code/MMD_NPU_code/MMD_NPU_depth_2_rgb/MMD_NPU/test_res/vis",
#             # obj_gen_type = obj_gen_type,
#             seed=0, cfg=3.0)
