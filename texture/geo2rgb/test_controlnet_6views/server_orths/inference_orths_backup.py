...
def render_obj_depth_normal(obj_path, out_dir, render_size=512, use_blender_coord=True, use_ortho = False):
    ...
    xyz_imgs[:,:,:,0][mask_xyz==0] = 0
    xyz_imgs[:,:,:,1][mask_xyz==0] = 0
    xyz_imgs[:,:,:,2][mask_xyz==0] = 0
...
def run_xyz2rgb(
    ...
    controlnet = ControlNetModel.from_pretrained(
        "/aigc_cfs/xibinsong/code/z123_gray/diffusers_triplane/configs/zero123plus/zero123plus_v24_4views/checkpoint-20000/controlnet", torch_dtype=torch.float16
        )
    ...
    normal, xyz = render_obj_depth_normal(obj_path=obj_path,out_dir=out_dir, render_size=1024, use_ortho=True)
    # breakpoint()
    ...
    result = pipeline(cond, depth_image=depth, controlnet=controlnet, guidance_scale=i, conditioning_scale=conditioning_scale, num_inference_steps=75, width=1024, height=1024).images[0]
    # result.show()
    ...
    if vis_dir is not None:
        ...
        in_img = torch.cat([ref_img, (depths.clip(0,1) * 255).permute(2,0,3,1).reshape(512,4*512,3)], dim=1)
