...
two_levels_up_src = os.path.abspath(os.path.join(two_levels_up, "src"))
sys.path.append(two_levels_up)

from src.diffusers.pipelines.zero123_diffusion.pipeline_zero123plus_v2 import Zero123PlusPipeline, RefOnlyNoisedUNet, scale_image, scale_latents, unscale_image, unscale_latents
...
def parse_args():
    ...
    parser.add_argument(
        "--empty_prompt_embedding_path",
        type=str,
        default=None,
        required=True,
        help='empty_prompt_embedding.'
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
...
def main():
    ...
    # controlnet_path = "/aigc_cfs/model/controlnet-zp11-depth-v1"
    controlnet_path = args.pretrained_controlnet_model_name_or_path
    ...
    # weight_dtype_fp16 = torch.float16
    vae.to(accelerator.device, dtype=weight_dtype)
    ...
    #         global_step,
    #     )
    # empty_prompt_embedding = torch.load("/aigc_cfs_2/neoshang/code/diffusers_triplane/configs/zero123plus/zero123plus_v24_4views/empty_prompt_embedding.pt")
    empty_prompt_embedding = torch.load("/aigc_cfs/xibinsong/models/empty_prompt_embedding.pt")
    pre_param_grad = None
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                controlnet_image = batch["xyzs_out"].to(dtype=weight_dtype)

                # print("controlnet_image: ", controlnet_image.shape)
                # breakpoint()

                # print("device:", accelerate.device)

                cond_image_clip = batch["cond_image_clip"]
                cond_image_vae = batch["cond_images_vae"].to(weight_dtype).squeeze()
                images_out = batch["images_out"]
                images_out = scale_image(images_out)
                b, vio, c, h, w = images_out.shape
                # end_time_1 = time.time()
                # print("11111: ", end_time_1 - start_time_1)
                # breakpoint()
                # ### get latents out
                # ncol = v // 2
                # nrow = v // ncol
                ...
                # print("555555:", end_time_1 - start_time_1)
                # breakpoint()
                latents_out.to(weight_dtype)
                ...
                # end_time_1 = time.time()
                # print("2222: ", end_time_1 - start_time_1)

                for i in range(b):
                    control_img_grids.append(make_grid(controlnet_image[i], nrow=nrow, padding=0))
                        # control_img_grids.append(controlnet_image[i])
                latents_out_grid = latents_out_grid * vae.config.scaling_factor
                latents_out_grid = scale_latents(latents_out_grid)

                control_img_grids = torch.stack(control_img_grids, dim=0)

                if accelerator.is_main_process and (global_step == 0):
                    print(latents_out_grid.shape)
                # prompt_embedding = tokenize_captions([""]).to(cond_image_vae.device)
                # prompt_embedding_batch = torch.cat([prompt_embedding] * b, dim=0)
                # encoder_hidden_states_prompt = text_encoder(prompt_embedding_batch)[0]
                encoder_hidden_states_prompt = torch.cat([empty_prompt_embedding.to(latents_out_grid.device)] * b, dim=0)
                ...
                # print("here !!!")
                # print(cond_image_vae.shape)
                # breakpoint()

                # end_time_1 = time.time()
                # print("33333: ", end_time_1 - start_time_1)

                # latents_cond_vae = vae.encode(cond_image_vae.to(weight_dtype)).latent_dist.sample()
                latents_cond_vae = vae.encode(cond_image_vae.to(weight_dtype)).latent_dist.sample()
                # latents_cond_vae.to(weight_dtype_fp16)
                # print("here 22222 !!!")
                # print(latents_cond_vae.shape)
                cross_attention_kwargs = dict(cond_lat=latents_cond_vae)


                # end_time_2 = time.time()
                # print("444444: ", end_time_2 - start_time_1)
                ...
                encoder_hidden_states_control = unet.ramping_add(encoder_hidden_states_prompt, cond_encoded_clip)
                encoder_hidden_states_control = encoder_hidden_states_control.to(weight_dtype)
                encoder_hidden_states_prompt = encoder_hidden_states_prompt.to(weight_dtype)

                down_block_res_samples, mid_block_res_sample = controlnet(
                    noisy_latents,
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states_control,
                    controlnet_cond=control_img_grids,
                    return_dict=False,
                )
                ...
                # print(encoder_hidden_states_prompt.dtype)
                # print(cond_encoded_clip.dtype)
                # print(encoder_hidden_states_control.dtype)
