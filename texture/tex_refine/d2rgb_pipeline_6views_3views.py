...
class RefOnlyNoisedUNet(torch.nn.Module):
    ...
    def forward_cond(self, noisy_cond_lat, timestep, encoder_hidden_states, class_labels, ref_dict, is_cfg_guidance, **kwargs):
        ...
        print("[DEBUG] Line 173 unet : ", type(self.unet))

        self.unet.set_is_controlnet(is_controlnet=False)

        print("[DEBUG] Line 175 cross_attention_kwargs created as : ", dict(mode="w", ref_dict=ref_dict).keys())
        # [DEBUG] Line 175 cross_attention_kwargs created as :  {'mode': 'w', 'ref_dict': {}}
        print("[DEBUG] Line 177 unet : ", type(self.unet))
        self.unet(
    ...
    def forward(
        ...
        **kwargs
        ...
        print("[DEBUG] Line 276 cross_attention_kwargs original: ", cross_attention_kwargs.keys())
        # [DEBUG] Line 276 cross_attention_kwargs original:  {'cond_lat': tensor([[[[[ 1.5688e+01,
        print("[DEBUG] Line 276 cross_attention_kwargs created as : ", dict(mode="r", ref_dict=ref_dict, is_cfg_guidance=is_cfg_guidance).keys())
        # [DEBUG] Line 276 cross_attention_kwargs created as :  {'mode': 'r', 'ref_dict': {'down_blocks.0.attentions.0.transformer_blocks.0.attn1.processor': tensor([[[ 1.5430e-01,
        print("[DEBUG] Line 276 unet : ", type(self.unet))
        return self.unet(
            sample, timestep,
...
class DepthControlUNet(torch.nn.Module):
    ...
    def forward(self, sample, timestep, encoder_hidden_states, class_labels=None, *args, cross_attention_kwargs: dict, **kwargs):
        ...
        print(cross_attention_kwargs.keys())
...
class Zero123PlusPipeline(diffusers.StableDiffusionPipeline):
    ...
    def __call_sd__(
        ...
        **kwargs,
        ...
        print("[DEBUG] Line 641 cross_attention_kwargs: ", cross_attention_kwargs.keys())
        print("[DEBUG] Line 641 unet : ", type(self.unet))
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                print("[DEBUG] Line 778 cross_attention_kwargs: ", cross_attention_kwargs.keys())
                print("[DEBUG] Line 778 unet : ", type(self.unet))
    ...
    def __call__(
        ...
        **kwargs
        ...
        # conditioning_scale = 1.5
        print("[DEBUG] line 961 self.unet: ",type(self.unet)) # RefOnlyNoisedUNet
        if type(self.unet) != DepthControlUNet:
            self.unet = DepthControlUNet(self.unet, controlnet, conditioning_scale)
        self.unet = self.unet.to(self.vae.device, dtype=self.vae.dtype)
        print("[DEBUG] line 961 self.unet: ",type(self.unet)) # DepthControlUNet
        # self.unet.conditioning_scale = conditioning_scale
        if image is None:
            raise ValueError("Inputting embeddings not supported for this pipeline. Please pass an image.")
        assert not isinstance(images_list[0], torch.Tensor)
        # print("111111")
        ...
        print("[DEBUG] Line 1018 cross_attention_kwargs: ", cross_attention_kwargs.keys())
        print("[DEBUG] Line 1018 unet : ", type(self.unet))
