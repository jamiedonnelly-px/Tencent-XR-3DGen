...
class ConsistentEulerAncestralDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def init_renderer(self, 
        ...
        bake_erode_boundary = 3

        bake_top_view_weight = 0.2
        bake_bottom_view_weight = 0.2
        ...
        weights[4] *= bake_top_view_weight
        weights[5] *= bake_bottom_view_weight
        ...
        self.disable_rgb_clamp = disable_rgb_clamp
    ...
    def reuse_renderer(self, 
        ...
        print("[CONSISTENT_SCHEDULER] Reusing renderer from:", prev_renderer)
        ...
        renderer.modify_res(render_res=render_size, tex_res=texture_resolution)
        renderer.set_cameras(azimuths=cam_azimuths, elevations=cam_elevations, dists=cam_distances, camera_type=camera_type, zooms=1.0, near=1e-1, far=1e1)
        ...
        print("Renderer reused from previous one.")
    ...
    # ):
    #     breakpoint()
    #     in_images = torch.nn.functional.interpolate(in_images, size=None, scale_factor=self.renderer_extra_scale_up_factor, mode='bilinear', align_corners=None, recompute_scale_factor=None, antialias=False)
    ...
    #     color, alpha = self.renderer.sample_texture(texture, max_mip_level=None)
    ...
    def step(
        ...
        if apply_consistency:
            ...
            image_weights = torch.cat((in_images, self.weights), dim=-1)
            # DEBUG
            # print("[DEBUG] image_weights shape: ",image_weights.shape)
            # device = torch.device('cuda:0')
            # free, total = torch.cuda.mem_get_info(device)
            # mem_total_mb = total / 1024 ** 2
            # mem_free_mb = free / 1024 ** 2
            # mem_used_mb = (total - free) / 1024 ** 2
            # print(f"[DEBUG] Memory total: ",mem_total_mb)
            # print(f"[DEBUG] Memory free: ",mem_free_mb)
            # print(f"[DEBUG] Memory used: ",mem_used_mb)
            # texture_weights = self.renderer.bake_textures_raycast(image_weights, interpolation="nearest", inpaint=False)

            if self.upsample_final_views and self._step_index == self.num_inference_steps-1:
                texture_weights = self.renderer.bake_textures(image_weights, antialias=True, ssaa=False, inpaint=False)
            else:
            ...
            self.last_texture_map = torch.clamp(texture[0],0,1)
