...
class ConsistentEulerAncestralDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def bake_pbr_mr(
        ...
        in_images = torch.nn.functional.interpolate(in_images.float(), size=None, scale_factor=self.renderer_extra_scale_up_factor, mode='bilinear', align_corners=None, recompute_scale_factor=None, antialias=False)

        if in_images.shape[2] == in_images.shape[3]: # 4 views
            image_list = self.split_image_tensor(in_images, 2, 2)
            in_images = torch.stack([image_list[0],image_list[2],image_list[1],image_list[3],],dim=0)
        elif in_images.shape[2] == in_images.shape[3]*1.5: # 6 views
            image_list = self.split_image_tensor(in_images, 2, 3)
            in_images = torch.stack([image_list[0],image_list[3],image_list[1],image_list[4],image_list[2],image_list[5]],dim=0)

        # New baking
        # bake
        image_weights = torch.cat((in_images.permute(0,2,3,1), self.weights), dim=-1)
        ...
        texture_mr = self.renderer.inpaint_textures(texture, (total_weights<=1e-5), inpaint_method="laplace") # (1, img_res, img_res, 3)

        ### Render ###
        # re-render image from textures
        color, alpha = self.renderer.sample_texture(texture_mr, max_mip_level=4)
        ...
        if save_intermediate_results:
            plt.imsave(os.path.join(self.output_path,f"metallic_roughness_rendered.png"),(torch.clamp(metallic_roughness_z_rendered_grid[0].to(torch.float32),0,1).permute(1,2,0).contiguous().cpu().numpy()))
    ...
    def step(
        ...
        if apply_consistency:
            ...
            pred_rgb_x0 = torch.nn.functional.interpolate(pred_rgb_x0, size=None, scale_factor=self.renderer_extra_scale_up_factor, mode='bicubic', align_corners=None, recompute_scale_factor=None, antialias=False)
            ...
            if pred_rgb_x0.shape[2] == pred_rgb_x0.shape[3]: # 4 views
                image_list = self.split_image_tensor(pred_rgb_x0, 2, 2)
                in_images = torch.stack([image_list[0],image_list[2],image_list[1],image_list[3],],dim=0)
            elif pred_rgb_x0.shape[2] == pred_rgb_x0.shape[3]*1.5: # 6 views
                image_list = self.split_image_tensor(pred_rgb_x0, 2, 3)
                in_images = torch.stack([image_list[0],image_list[3],image_list[1],image_list[4],image_list[2],image_list[5]],dim=0)
            ...
            # plt.imsave("view0.png",in_images[0].data.cpu().numpy())
            # plt.imsave("view1.png",in_images[1].data.cpu().numpy())
            # plt.imsave("view2.png",in_images[2].data.cpu().numpy())
            # plt.imsave("view3.png",in_images[3].data.cpu().numpy())
            # plt.imsave("view4.png",in_images[4].data.cpu().numpy())
            # plt.imsave("view5.png",in_images[5].data.cpu().numpy())


            # plt.imsave("weight0.png",self.weights[0,:,:,0].data.cpu().numpy())
            # plt.imsave("weight1.png",self.weights[1,:,:,0].data.cpu().numpy())
            # plt.imsave("weight2.png",self.weights[2,:,:,0].data.cpu().numpy())
            # plt.imsave("weight3.png",self.weights[3,:,:,0].data.cpu().numpy())
            # plt.imsave("weight4.png",self.weights[4,:,:,0].data.cpu().numpy())
            # plt.imsave("weight5.png",self.weights[5,:,:,0].data.cpu().numpy())
            # st()

            # texture_weights = self.renderer.bake_textures_raycast(image_weights, interpolation="bicubic", inpaint=False)
            texture_weights = self.renderer.bake_textures(image_weights, antialias=False, ssaa=False, inpaint=False)
            textures, weights = torch.split(texture_weights, (3,1), dim=-1)
            # blend textures by weights
            total_weights = torch.sum(weights, dim=0, keepdim=True) # (1, img_res, img_res, 1)
            ...
            if pred_rgb_x0.shape[2] == pred_rgb_x0.shape[3]: # 4 views
                ...
                new_pred_x0_rgb = torch.zeros((1,3,render_size*3,render_size*2),device="cuda",dtype=torch.float32)

                new_pred_x0_rgb[:,:,:render_size,:render_size] = textured_views_rgb[0][:3]*textured_views_rgb[0][-1:] + ((1-textured_views_rgb[0][-1:])*0.5)# Grey background
                new_pred_x0_rgb[:,:,:render_size,render_size:] = textured_views_rgb[1][:3]*textured_views_rgb[1][-1:] + ((1-textured_views_rgb[1][-1:])*0.5)
                new_pred_x0_rgb[:,:,render_size:2*render_size,:render_size] = textured_views_rgb[2][:3]*textured_views_rgb[2][-1:] + ((1-textured_views_rgb[2][-1:])*0.5)
                new_pred_x0_rgb[:,:,render_size:2*render_size,render_size:] = textured_views_rgb[3][:3]*textured_views_rgb[3][-1:] + ((1-textured_views_rgb[3][-1:])*0.5)
                new_pred_x0_rgb[:,:,2*render_size:,:render_size] = textured_views_rgb[4][:3]*textured_views_rgb[4][-1:] + ((1-textured_views_rgb[4][-1:])*0.5)
                new_pred_x0_rgb[:,:,2*render_size:,render_size:] = textured_views_rgb[5][:3]*textured_views_rgb[5][-1:] + ((1-textured_views_rgb[5][-1:])*0.5)
