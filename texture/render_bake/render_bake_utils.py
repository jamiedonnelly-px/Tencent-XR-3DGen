...
def sparse(A):
    ...
    nonzero_mask = A.values() != 0
    
    filtered_indices = A.indices()[:, nonzero_mask]
    filtered_values = A.values()[nonzero_mask]

    return torch.sparse_coo_tensor(
        filtered_indices, filtered_values, A.shape, device=A.device
    ).coalesce()
...
def sparse_cat(tensors):
    ...
    for tensor in tensors:
        ...
        row_offset += tensor.size(0)

    indices_concat = torch.cat(indices_list, dim=1)
    values_concat = torch.cat(values_list)
    return torch.sparse_coo_tensor(indices_concat, values_concat, (row_offset, ncols)).coalesce()


def remove_columns(A, columns):
    """
    Removes specific columns from a 2D PyTorch COO sparse tensor.
    """
    if not A.is_coalesced():
        A = A.coalesce()

    if columns.dtype == torch.bool:
        columns = torch.arange(A.shape[1], dtype=torch.long, device=A.device)[columns]
    columns = torch.unique(columns, sorted=True)
    mask = ~torch.isin(A.indices()[1], columns)

    filtered_indices = A.indices()[:, mask]
    filtered_values = A.values()[mask]

    all_columns = torch.arange(A.shape[1], dtype=torch.long, device=A.device)
    keep_columns = all_columns[~torch.isin(all_columns, columns)]

    old_col_indices = filtered_indices[1]

    new_col_indices = torch.searchsorted(keep_columns, old_col_indices)
    filtered_indices[1] = new_col_indices

    return torch.sparse_coo_tensor(
        filtered_indices, filtered_values, (A.size(0), keep_columns.size(0))
    ).coalesce()
    
def find_seams(f, ft, assert_manifold=False):
    '''
    find uv seams, i.e. geometry edges that are splitted in uv,
    return a vt indexing tensor of shape [n_seams, edge_copies=2, vt_per_edge=2] representing UV seams
    
    this function assumes oriented closed manifold mesh, that is every edge is shared by exactly 2 triangles in f and
    surface has consistent orientation. otherwise bahaviour is undetermined. you can assert this is condition is met by 
    setting assert_manifold=True. TODO: support non-manifold meshes
    '''
    
    fe = torch.cat([f[:, [0, 1]], f[:, [1, 2]], f[:, [2, 0]]], dim=0) # [3xF, 2]
    fet = torch.cat([ft[:, [0, 1]], ft[:, [1, 2]], ft[:, [2, 0]]], dim=0) # [3xF, 2]

    flip = fe[...,0] > fe[...,1] # [3xF]
    ufe = torch.sort(fe, dim=1).values # [3xF, 2] undirected edge
    
    # _, cnt = torch.unique(ufe, dim=0, return_counts=True)
    # st()
    
    ufe_id = ufe[:,0]*(ufe.max()+1)+ufe[:,1] # [3xF]
    same_eid = torch.argsort(ufe_id) # [3Xf]
    same_mask = torch.nonzero(ufe_id[same_eid[:-1]] == ufe_id[same_eid[1:]]).flatten()
    same_eid = torch.stack((same_eid[same_mask], same_eid[same_mask+1]), dim=-1) # [ne, edge_copies=2] values in [0,3xF)
    
    # flip = fe[...,0] > fe[...,1] # [3xF]
    # ufe = torch.sort(fe, dim=1).values # [3xF, 2] undirected edge
    # ufe_id = ufe[:,0]*(ufe.max()+1)+ufe[:,1] # [3xF]
    # same_eid = torch.argsort(ufe_id) # [3Xf]
    # same_eid = same_eid.reshape(-1,2)
    
    fet = fet[same_eid] # [ne, edge_copies=2, vt_per_edge=2]
    fet[flip[same_eid]] = torch.flip(fet[flip[same_eid]], dims=(1,))
    seam_mask = torch.logical_or(fet[:,0,0] != fet[:,1,0], fet[:,0,1] != fet[:,1,1])
    seams = fet[seam_mask] # [n_seam, edge_copies=2, vt_per_edge=2]
    seams = seams.reshape(-1,4)[:,[0,1,3,2]].reshape(-1,2,2)

    return seams

def coords2D_to_sparse(coords, size):
    '''
    coords: tensor of shape [N,2], last dimension are the xy coordinates in region [0,W]x[0,H]
    size: tuple of integers (H,W), image size
    
    returns a sparse matrix of shape [N, H*W] that records the bilinear interpolation weights.
    For every 2D coordinate, its four corners in this sparse matrix are non-zero and record the bilinear weights.
    '''
    H, W = size
    N = coords.shape[0]
    
    coords = coords - 0.5 # shift from pixel corner to pixel centre
    ...
    top_right = y0 * W + x1
    bottom_left = y1 * W + x0
    bottom_right = y1 * W + x1
    
    col_indices = torch.cat([top_left, top_right, bottom_left, bottom_right])
    row_indices = torch.arange(N, device=coords.device).repeat(4)
    ...
    assert row_indices.max() < N and row_indices.min() >= 0 and col_indices.max() < H*W and col_indices.max() >= 0
    ...
    return sparse(sparse_matrix)


def kernel2D_to_sparse(kernel, weights):
    '''
    kernel: tensor of shape [h, w], image filtering kernel
    weights: tensor of shape (H, W), weights

    returns a sparse matrix A of shape [N, H*W] that records the image filtering operation, 
    where N is the number of non-zero entries in weights, such that A @ Image.flatten() is equivalent to 
    filter(image, kernel)[mask]. assumes border padding TODO: support more padding modes
    '''
    ...
    positions = torch.nonzero(weights, as_tuple=False)  # [N, 2]
    try:
        assert positions[:,0].max() < H and positions[:,0].min() >= 0 and positions[:,1].max() <= W and positions[:,1].min() >= 0
    except:
        st()
    nonzero_weights = weights[positions[:,0], positions[:,1]] # [N]
    N = positions.shape[0]

    ki = torch.arange(h, device=device)
    kj = torch.arange(w, device=device)
    kernel_offsets = torch.stack(torch.meshgrid(ki, kj, indexing='ij'), dim=-1).reshape(-1, 2) - torch.tensor([pad_h, pad_w], device=device)
    k = kernel_offsets.shape[0] 

    positions_expanded = positions[:, None, :]  # [N, 1, 2]
    kernel_offsets_expanded = kernel_offsets[None, :, :]  # [1, k, 2]
    image_coords = positions_expanded + kernel_offsets_expanded  # [N, k, 2]
    
    image_i = image_coords[:, :, 0].clip(0,H-1).long() # [N, k]
    image_j = image_coords[:, :, 1].clip(0,W-1).long()
    
    valid = (weights[image_i, image_j] != 0) # [N, k]
    empty = (valid == False)
    image_i[empty] = positions_expanded[...,0].expand_as(image_i)[empty] # pad borders
    image_j[empty] = positions_expanded[...,1].expand_as(image_j)[empty] # pad borders

    col_indices = image_i * W + image_j
    row_indices = torch.arange(N, device=device).unsqueeze(1).expand(N, k)
    values = kernel.reshape(1,-1) * (nonzero_weights).reshape(-1,1) # [N,k]
    # values[empty] *= 0
    
    assert torch.all((weights != 0).reshape(-1)[col_indices])

    indices = torch.stack([row_indices, col_indices], dim=0)  # [2, N, k]
    
    assert row_indices.max() < N and row_indices.min() >= 0 and col_indices.max() < H*W and col_indices.max() >= 0
    A = torch.sparse_coo_tensor(indices.reshape(2,-1), values.flatten(), size=(N, H * W)).coalesce()
    
    return sparse(A), valid


def build_seam_smoothness_term(seams, vt, uv_res, rel_density=1.0):
    '''
    returns a sparse matrix A such that texture consistency along seams is preserved by solving A@I_tex=0  
    '''
    svt = vt[seams] * uv_res # [n_seam, edge_copies=2, vt_per_edge=2, uv=2]
    edge_lengths = torch.linalg.vector_norm(svt[:,:,0] - svt[:,:,1], dim=2) # [n_seam, edge_copies=2]
    seam_samples = edge_lengths * rel_density # [n_seam, edge_copies=2]
    seam_samples = torch.ceil(seam_samples.max(1).values).int() # [n_seam]
    
    steps = torch.repeat_interleave(torch.arange(len(seam_samples)).to(seam_samples), seam_samples)
    steps = torch.arange(seam_samples.sum()).to(seam_samples) - torch.cat([torch.tensor([0]).to(seam_samples), seam_samples.cumsum(0)])[:-1][steps.long()]
    total_steps = seam_samples.repeat_interleave(seam_samples.int())
    norm_steps = (steps.float() + 1) / (total_steps.float() + 1) # [n_samples]
    
    v0, v1 = torch.split(svt, 1, dim=2) # [n_seam, edge_copies=2, 1, uv=2]
    v0 = v0.squeeze(2).repeat_interleave(seam_samples.int(), dim=0) # [n_samples, edge_copies=2, uv=2]
    v1 = v1.squeeze(2).repeat_interleave(seam_samples.int(), dim=0) # [n_samples, edge_copies=2, uv=2]
    samples = v0 + (v1-v0) * norm_steps.reshape(-1,1,1) # [n_samples, edge_copies=2, uv=2]
    
    return coords2D_to_sparse(samples[:,0], (uv_res,uv_res)), coords2D_to_sparse(samples[:,1], (uv_res,uv_res))
    
def build_data_term(uv_tex, uv_weights, ord=0):
    '''
    returns a sparse matrix A and corresponding matrix b such that data prior is preserved through A@I_tex=b
    
    uv_tex: shape [tex_res, tex_res, channels]
    uv_weights: [tex_res, tex_res]
    ord: poisson equation order, 0 for absolute scale or 1 for graident or 'laplace'
    
    returns sparse matrix A of shape [N, tex_res^2] where N is determined by number of non-zero entries in uv_weights and order
    '''
    dtype = uv_tex.dtype
    device = uv_tex.device
    channels = uv_tex.shape[-1]
    
    if ord == 0:
        kernel = torch.ones((1,1), device=device, dtype=dtype)
        A,m = kernel2D_to_sparse(kernel, uv_weights)
    if ord == 1:
        kernel_h = torch.tensor([-0.5,0,0.5], device=device, dtype=dtype).reshape(1,3)
        kernel_v = torch.tensor([-0.5,0,0.5], device=device, dtype=dtype).reshape(3,1)
        A_h,m_h = kernel2D_to_sparse(kernel_h, uv_weights)
        A_v,m_v = kernel2D_to_sparse(kernel_v, uv_weights)
        A = sparse_cat((A_h, A_v))
        m = torch.cat((m_h, m_v))
    if ord == 'laplace':
        kernel = torch.tensor([
            [0, 1, 0],
    ...
    return A, A @ uv_tex.reshape(-1, channels) #* m.reshape(-1,1)
...
def sparse_eye(N, device=None, dtype=torch.float32):
    ...
    return torch.sparse_coo_tensor(indices, values, (N, N), device=device, dtype=dtype).coalesce()

def sparse_transpose(A):
    return torch.sparse_coo_tensor(
        A.indices().flip((0,)), A.values(), A.shape[::-1]
    ).coalesce()

def sparse_cg(A, b, damp=0.0, maxiter=1000, tol=1e-6, x0=None):
    ...
    if x0 is not None:
        ...
        x = torch.zeros((N, b.size(1)), device=A.device)
    r = Atb.clone()
    p = r.clone()
...
def sparse_gd(A, b, damp=0.0, maxiter=1000, tol=1e-6, x0=None, solver="SGD", **optim_params):
    ...
    with torch.enable_grad():
        ...
        optim = getattr(torch.optim, solver)([x], **optim_params)
        # optim = torch.optim.SGD([x], **optim_params)
        ...
        for i in range(maxiter):
            ...
            def closure():
                optim.zero_grad()
                r = A @ x - b
                l = torch.sum(r*r) + torch.sum(x*x) * damp
                return l
...
def solve_lsqr(A, b, method="cg", x0=None, solver_params=dict()):
    '''
    solves sparse linear system with min||A@x-b||2 where A is MxN sparse and b is MxC dense
    returns solution x NxC dense 
    '''
    # remove unconstrained columns
    A = sparse(A)
    colmask = torch.zeros(A.shape[1], dtype=torch.bool, device=A.device)
    colmask[A.indices()[1]] = True
    A = remove_columns(A, colmask==False)
    if x0 is not None:
        x0 = x0[colmask]
    ...
    if method in ["lsmr","lsqr"]:
        ...
        solver = lsqr if method == "lsqr" else lsmr
        A_scipy = coo_matrix((A.values().cpu().numpy(),
                            (A.indices()[0].cpu().numpy(), A.indices()[1].cpu().numpy())),
                            shape=A.shape)
        b_numpy = b.cpu().numpy()
        
        if "maxiter" in solver_params and method=="lsqr":
            solver_params = dict(**solver_params)
            solver_params["iter_lim"] = solver_params["maxiter"]
            del solver_params["maxiter"]
            
        if x0 is not None:
            x0 = x0.cpu().numpy()
        
        if b_numpy.ndim == 2:
            x_numpy = np.zeros((A.shape[1], b_numpy.shape[1]), dtype=np.float32)
            for i in range(b_numpy.shape[1]):
                if x0 is None:
                    x_numpy[:,i] = solver(A_scipy, b_numpy[:,i], **solver_params)[0]
                else:
                    x_numpy[:,i] = solver(A_scipy, b_numpy[:,i], x0=x0[:,i], **solver_params)[0]
        else:
            x_numpy = solver(A_scipy, b_numpy, x0=x0, **solver_params)[0]
        x = torch.from_numpy(x_numpy).to(b)
    elif method == "cg":
        x = sparse_cg(A, b, x0=x0, **solver_params)
        
    else:
        x = sparse_gd(A, b, solver=method, x0=x0, **solver_params)
    
    x_full = torch.zeros((colmask.shape[0], x.shape[1]), dtype=x.dtype, device=x.device)
    x_full[colmask] = x
    return x_full

class UVPoisson:
    
    def __init__(self, tex_channels, f, vt, ft, uv_res):
        self.uv_res = uv_res
        self.f = f
        self.vt = vt
        self.ft = ft
        self.channels = tex_channels
        self.seams = find_seams(self.f, self.ft, assert_manifold=False)
        
        i = torch.arange(self.uv_res, device=self.f.device)
        self.uv_idx = sum(torch.meshgrid(i*self.uv_res, i, indexing='ij')) # [res, res]
    ...
    def add_data_prior(self, tex, weights, ord=0):
        '''
        Add data terms to poisson equation system
        - tex: shape [res, res, channels]
        - weights: shape [res, res]
        '''
        assert tex.shape == (self.uv_res, self.uv_res, self.channels)
        assert weights.shape == (self.uv_res, self.uv_res)
        if (weights != 0).sum() > 0:
            A, b = build_data_term(tex, weights, ord)
            self.A.append(A)
            self.b.append(b)
        
    def add_seam_smoothness(self, density=1.0, weight=1.0):
        if density * weight == 0:
            return
        A1, A2 = build_seam_smoothness_term(self.seams, self.vt, self.uv_res, density)
        self.A.append((A1-A2) * weight)
        self.b.append(torch.zeros((A1.shape[0], self.channels)).to(self.vt))
        
    def add_constraints(self, tex, mask):
        if mask.sum() > 0:
            self.ci.append(self.uv_idx[mask]) # [nc]
            self.cv.append(tex[mask]) # [nc, channel]
    
    def solve(self, method="lsmr", x0=None, solver_params=dict()):
        assert len(self.A) > 0
        
        A = sparse_cat(self.A)
        b = torch.cat(self.b)
        mask = torch.ones(self.uv_res**2, dtype=torch.bool, device=b.device)
        
        if len(self.ci) > 0:
            ci = torch.cat(self.ci)
            cv = torch.cat(self.cv)
            cvb = torch.zeros(self.uv_res**2, dtype=cv.dtype, device=b.device)
            cvb[ci] = cv
            b = b - A @ cvb
            A = remove_columns(A, ci)
            mask[ci] = False
        
        if x0 is not None:
            x0 = x0.reshape(self.uv_res**2, self.channels)[mask]
            
        x = solve_lsqr(A, b, method, x0, solver_params)
        
        tex = torch.zeros(self.uv_res**2, self.channels, dtype=b.dtype, device=b.device)
        tex[mask] = x
        if len(self.ci) > 0:
            tex[ci] = cv
        return tex.reshape(self.uv_res, self.uv_res, self.channels)

def build_poisson_equations_and_solve(mesh, textures, poisson_weights, seam_consistency_weight, seam_density=1.0, solver="cg", init_solution=None, solver_params=dict()):
    f = mesh.f
    vt = mesh.vt[0]
    ft = mesh.ft
    n_views, uv_res, uv_res, n_channels = textures.shape
    
    w0, w1, wl, wc = poisson_weights.permute(3,0,1,2) # [n_views, uv_res, uv_res]
    
    poisson_sys = UVPoisson(n_channels, f, vt, ft, uv_res)
    
    if seam_consistency_weight > 0:
        poisson_sys.add_seam_smoothness(density=seam_density, weight=seam_consistency_weight)
    
    wc = wc / (wc.sum(0, keepdim=True) + 1e-10) # normalize in case multiple entires per pixel
    poisson_sys.add_constraints((textures * wc.unsqueeze(-1)).sum(0), wc.sum(0)>0)
    
    for t, w in zip(textures, w0):
        poisson_sys.add_data_prior(t, w, 0)
    
    for t, w in zip(textures, w1):
        poisson_sys.add_data_prior(t, w, 1)
        
    for t, w in zip(textures, wl):
        poisson_sys.add_data_prior(t, w, "laplace")
    
    ret = poisson_sys.solve(solver, init_solution, solver_params)
...
def construct_mesh_laplacian(faces, diagonal_val=-1):
    ...
    return laplacian_matrix.coalesce()
...
class Mesh():
    ...
    def unwrap_uv(self, uv_res, padding=2, max_stretch=0.15, parallel_regions=1):
...
class Renderer:
    def __init__(self, render_res=1024, tex_res=1024, ssaa=1, world_orientation="y-up", rast_ctx="cuda", rast_tile_size=None):
        '''
        init the renderer
    ...
    def unwrap_uv(self, padding=2, max_stretch=0.15, parallel_regions=1):
        self.mesh.unwrap_uv(self.tex_res, padding, max_stretch, parallel_regions)
        self._clear_cache(clear_uv=True)
    ...
    def overwrite_mtl(path, texture, val_range=(0,1), texture_prefix="texture", default_material_name="raw_mesh"):
        ...
        new_mtl_content = (
            f"# {'Overwritten' if lines else 'Created'} for PBR workflow" + "\n" +
    ...
    def solve_poisson_multigrid(self, textures, poisson_weights, seam_consistency_weight=0.1, seam_density=1.0, multigrid_stages={}):
        
        '''
        blends multiview textures by solving a poisson system
        
        Inputs:
        - textures: numpy or torch, of shape [n_views, tex_res, tex_res, n_channels]
        - poisson_weights: weights for poisson equation numpy or torch, of shape [n_views, tex_res, tex_res, 4], number of non-zero elements increases computation overhead.
        the last 4 channels
            - 0th channel: weight for the value (i.e. greater means final texture value should be close to corresponding value in input texture)
            - 1st channel: weight for the gradient (i.e. greater means final texture's spatial gradients should be close to corresponding input texture gradients)
            - 2nd channel: weight for the laplacian (i.e. greater means final texture's laplacian should be close to corresponding input texture laplacian)
            - 3rd channel: constraints (i.e. this channel contains binary values that if set to 1, the output texture will take the corresponding value in input texture and will not be optimised)
        - seam_consistency_weight: float, consistency weights to enforce same texture across UV seams
        - seam_density: float, sampling rates on UV seams in n_samples per textel, increases computation overhead
        - multigrid_stages: a list of dictionary each representing a multigrid solver stage, e.g.
            multigrid_stages = [
                dict(uv_res=256, solver="LBFGS", solver_params=dict(maxiter=1000, damp=1e-3)), # initial stage at resolution 256, use LBFGS solver for 1000 iters and add 0.001 dampening
                dict(uv_res=512, solver="AdamW", solver_params=dict(maxiter=100, lr=1e-1, damp=1e-6)), # next stage upsamples from 256 to 512, and uses AdamW solver
                ...
                dict(uv_res=4096, solver="AdamW", solver_params=dict(maxiter=10, lr=2e-2, damp=1e-6)), # final stage, whose uv_res should be same as renderer.tex_res
            ]
            
        Returns:
            - texture: torch, of shape [tex_res, tex_res, n_channels]. texture[i,j,:]=0 where poisson_weights[:,i,j,:] are all zeros
        '''

        x = None
        
        textures, poisson_weights = cvt_torch(textures, poisson_weights)
        
        for optim_stage in multigrid_stages:
            
            uv_res = min(optim_stage["uv_res"], textures.shape[1])
            
            if uv_res != textures.shape[1]:
                tex = torch.nn.functional.interpolate(textures.permute(0,3,1,2), (uv_res, uv_res), mode="nearest").permute(0,2,3,1)
                wgh = torch.nn.functional.interpolate(poisson_weights.permute(0,3,1,2), (uv_res, uv_res), mode="nearest").permute(0,2,3,1)
            else:
                tex = textures
                wgh = poisson_weights
                
            if x is not None:
                x = torch.nn.functional.interpolate(x.permute(2,0,1).unsqueeze(0), (uv_res, uv_res)).permute(0,2,3,1)[0]
            
            x = build_poisson_equations_and_solve(self.mesh, tex, wgh, seam_consistency_weight, seam_density, optim_stage["solver"], x, optim_stage["solver_params"])
            
            mask = (wgh.abs().sum((0,3)) > 0).unsqueeze(-1)
            x = self._voronoi_inpaint(x.unsqueeze(0), mask.unsqueeze(0)).squeeze(0)
