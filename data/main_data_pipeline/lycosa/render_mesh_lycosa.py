...
if __name__ == '__main__':
    ...
    if len(data_json_path) > 1:
        mesh_path_list, manifold_mesh_path_list, input_proc_data_folder_list, _, category_list, instance_name_list = read_mesh_list_from_data_json(
            data_json_path)
    ...
    for index in range(len(mesh_path_list)):
        ...
        if apply_render:
            ...
            if config_struct["light_source"] == "hdr":
                ...
                if hdr_usage == 'random':
                    ...
                    if "hdr_path" in config_struct:
                        hdr_map_path = os.path.join(os.path.dirname(
                            os.path.abspath(__file__)), config_struct["hdr_path"])
            ...
            for render_stage in render_stages:
                ...
                if render_stage == "roughness":
                    render_stage_map["cmd"][
                        render_stage] = render_cmd + " --render_material --material_type \"Roughness\" "
                if render_stage == "metallic":
                    render_stage_map["cmd"][
                        render_stage] = render_cmd + " --render_material --material_type \"Metallic\"  "
                if render_stage == "common":
                ...
                if render_stage == "emission" or render_stage == "equilibrium":
                    render_stage_map["cmd"][render_stage] = render_stage_map["cmd"][render_stage] + \
                                                            " --only_render_png --smooth --no_camera_export "
                if render_stage == "roughness" or render_stage == "metallic":
                    render_stage_map["cmd"][render_stage] = render_stage_map["cmd"][render_stage] + \
                                                            " --only_render_png --smooth --no_camera_export "
