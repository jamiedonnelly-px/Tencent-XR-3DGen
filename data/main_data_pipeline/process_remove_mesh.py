...
def convert_once(bake_cmd, destination_mesh_path, output_folder, stat_txt, time_txt, folder_txt):
    stat = 0
    t_start = time.time()
    ...
    time.sleep(0.1)
    print("Start command %s: %s" % ("mesh remove", bake_cmd))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--remove_mesh_material_name', type=str, default="",
                        help='remove mesh with materials using this name')
    ...
    remove_mesh_material_name = args.remove_mesh_material_name
    ...
    if len(data_json_path) > 1:
        source_mesh_paths, mesh_categories, instance_name_list, _ = read_mesh_list_from_data_json(data_json_path)
    ...
    mesh_remover_fullpath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "geometry/remove_certain_mesh.py")
    ...
    for index in range(len(source_mesh_paths)):
        ...
        mesh_remove_cmd = "{} -b -P {} --".format(blender_root, mesh_remover_fullpath)
        mesh_remove_cmd = mesh_remove_cmd + " --mesh_path \"{}\" ".format(source_mesh_path)
        mesh_remove_cmd = mesh_remove_cmd + " --output_mesh_path \"{}\" ".format(final_output_path)
        mesh_remove_cmd = mesh_remove_cmd + " --remove_name_list_str \"{}\" ".format(remove_mesh_material_name)

        print("Remove material in mesh %s with name %s" % (source_mesh_path, args.remove_mesh_material_name))
        os.system(mesh_remove_cmd)

        pool.apply_async(func=convert_once,
                         args=(mesh_remove_cmd, source_mesh_path, final_output_path, stat_txt, time_txt, folder_txt))

        with open(cmds_txt, 'a') as f:
            f.write(mesh_remove_cmd + '\n')
