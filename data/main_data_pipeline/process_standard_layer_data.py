...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_json_path', type=str, default="",
                        help='output mesh info json path')
    parser.add_argument('--data_json_path', type=str, default="",
                        help='data json file path')
    parser.add_argument('--category_trans_json_path', type=str, default="",
                        help='category trans json file path')
    ...
    category_trans_json_path = args.category_trans_json_path
    ...
    if not os.path.exists(config_json_path):
        config_json_path = os.path.join(os.path.dirname(
            os.path.abspath(__file__)), "config/standard_data.json")

    config_data_struct = read_json(config_json_path)
    category_transtxt_map = read_json(category_trans_json_path)
    ...
    if args.pod_num >= 0 and args.pod_id >= 0:
        ...
        output_json_folder = os.path.split(args.output_json_path)[0]
        output_json_filename = os.path.split(args.output_json_path)[1]
        correct_output_json_folder = os.path.join(
            output_json_folder, "pod_{}".format(args.pod_id))
        if not os.path.exists(correct_output_json_folder):
            os.mkdir(correct_output_json_folder)
        output_json_path = os.path.join(
            correct_output_json_folder, output_json_filename)
    ...
    for index in range(len(mesh_paths)):
        ...
        if index < len(mesh_categories):
            ...
            mesh_parent_category = mesh_categories[index].replace(
                "_" + mesh_category, "")
        else:
            mesh_parent_folder = os.path.split(mesh_folder)[0]
            mesh_category = os.path.split(mesh_parent_folder)[1]
            mesh_data_name_folder = os.path.split(mesh_parent_folder)[0]
            mesh_parent_category = os.path.split(mesh_data_name_folder)[1]
        ...
        mesh_parent_category_folder = os.path.join(
            output_folder, mesh_parent_category)
        if not os.path.exists(mesh_parent_category_folder):
            os.mkdir(mesh_parent_category_folder)
        mesh_category_folder = os.path.join(
            mesh_parent_category_folder, mesh_category)
        ...
        transformation_txt = os.path.join(
            new_mesh_folder, "transformation.txt")

        if os.path.exists(transformation_txt):
            # in case of the src and dst are the same file
            if not os.path.samefile(category_transtxt_map[mesh_category], transformation_txt):
                os.remove(transformation_txt)
        shutil.copyfile(
            category_transtxt_map[mesh_category], transformation_txt)
        ...
        resize_cmd = "{} -b -P {} -- --mesh_path '{}' --resize_mesh_path '{}' --input_transformation_txt_path '{}' --copy_texture ".format(
        ...
        bake_cmd = bake_cmd + \
                   " --output_mesh_folder '{}' ".format(texture_mesh_folder)
        bake_cmd = bake_cmd + " --output_mesh_filename \"texture\" "
        bake_cmd = bake_cmd + \
    ...
    for index in range(len(result_folder_list)):
        ...
        instance_struct["ObjMesh"] = texture_mesh_name
    ...
    print("Standard layer data processing end. Start time is %s; end time is %s" %
