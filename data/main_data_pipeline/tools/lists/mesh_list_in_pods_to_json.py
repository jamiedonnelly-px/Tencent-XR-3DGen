...
if __name__ == '__main__':
    ...
    parser.add_argument('--pod_folder_path', type=str,
                        help='path to a group of folder txt file')
    parser.add_argument('--output_json_path', type=str,
                        help='path to generated json file')
    args = parser.parse_args()

    pod_folder_path = args.pod_folder_path
    output_json_path = args.output_json_path

    data_info = {}
    data_info["data"] = {}

    pod_folder_names = os.listdir(pod_folder_path)
    for pod_name in pod_folder_names:
        if "pod_" not in pod_name:
            continue
        pod_folder_fullpath = os.path.join(pod_folder_path, pod_name)
        mesh_list_txt_path = os.path.join(pod_folder_fullpath, "folder.txt")
        if not os.path.exists(mesh_list_txt_path):
            continue
        ...
        for mesh_path in mesh_path_list:
            ...
            # mesh_name = mesh_basename.replace("-", "_")
