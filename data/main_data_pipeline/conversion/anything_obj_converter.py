...
def load_mesh(mesh_path: str, force_old_importer: bool = False):
    bpy.ops.object.select_all(action='DESELECT')
    version_info = bpy.app.version
    if version_info[0] > 2:
        if force_old_importer:
...
if __name__ == '__main__':
    ...
    print("anything to obj conversion start. Local time is %s" % (start_time_str))

    argv = sys.argv
    raw_argv = argv[argv.index("--") + 1:]  # get all args after "--"

    parser = argparse.ArgumentParser(description='anything to obj data script.')
    ...
    parser.add_argument('--no_additional_texture', action='store_true',
                        help='add aditional texture to mesh with uv but without texture/material')
    parser.add_argument('--force_old_obj_importer', action='store_true',
                        help='force use old obj importing function')
    ...
    no_additional_texture = args.no_additional_texture
    force_old_obj_importer = args.force_old_obj_importer
    ...
    if mesh_extension == '.fbx':
        ...
        load_mesh(mesh_path, force_old_importer=force_old_obj_importer)
    ...
    if copy_texture:
        export_mesh_obj(joint_mesh, output_mesh_path, path_mode='COPY', z_up=force_z_up)
    else:
        export_mesh_obj(joint_mesh, output_mesh_path, path_mode='RELATIVE', z_up=force_z_up)
