...
if __name__ == '__main__':
    ...
    parser.add_argument('--copy_texture', action='store_true',
                        help='copy original texture file two new folder')
    parser.add_argument('--pack', action='store_true',
                        help='pack all texture images into fbx')
    parser.add_argument('--import_scale', type=float, default=1.0,
                        help='scale of fbx import')
    ...
    mesh_extention = os.path.splitext(mesh_path)[1]

    if mesh_extention == ".obj":
        meshes = load_mesh(mesh_path)
    elif mesh_extention == ".fbx":
        bpy.ops.import_scene.fbx(
            filepath=mesh_path, use_anim=True, global_scale=import_scale)
    ...
    if len(meshes) < 1:
        print("No mesh parts found in model.....")
        exit(-1)
    ...
    if args.pack:
        bpy.ops.export_scene.fbx(
            filepath=output_fullpath, path_mode='COPY', embed_textures=True)
    else:
        if args.copy_texture:
            bpy.ops.export_scene.fbx(
                filepath=output_fullpath, path_mode='COPY')
        else:
            bpy.ops.export_scene.fbx(filepath=output_fullpath)
