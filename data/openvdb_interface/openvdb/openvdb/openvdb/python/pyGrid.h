...
#include <openvdb/math/FiniteDifference.h> // for math::isGradient()
...
template<typename GridType>
inline std::tuple<py::object, py::object>
queryValGradInt(GridType& grid, py::object coordsArray)
{
    PyErr_SetString(PyExc_NotImplementedError, "this module was built without NumPy support");
    throw py::error_already_set();
    return py::object();
...
activeIndices(const GridType& grid)
{
    std::vector<Vec3I> coords;

    // activeLeafVoxelCount is a lower bound for pre-allocation, but if there are
    // large tiles at upper levels, final count can exceed this.

    // ValueOnIter visits all active values: both single voxels (in leaves) and tiles.
    for (auto iter = grid.cbeginValueOn(); iter; ++iter) {
        const openvdb::CoordBBox bbox = iter.getBoundingBox();
        const openvdb::Coord& minCoord = bbox.min();
        const openvdb::Coord& maxCoord = bbox.max();

        // Get the number of voxels spanned by this value (tile or voxel).
        const Index64 voxelCount = iter.getVoxelCount();

        if (voxelCount == 1) {
            // This is a single active voxel.
            coords.emplace_back(minCoord.x(), minCoord.y(), minCoord.z());
        } else {
            // This is a tile that covers multiple active voxels.
            // Expand it into all voxel coordinates in its bounding box.
    ...
    std::vector<ssize_t> shape = { static_cast<ssize_t>(coords.size()), 3 };
    std::vector<ssize_t> strides = { 3 * static_cast<ssize_t>(sizeof(int)), static_cast<ssize_t>(sizeof(int))};
    py::buffer_info coordInfo(coords.data(), sizeof(int), py::format_descriptor<int>::format(), 2, shape, strides);
    py::array_t<int> coordArray(coordInfo);

    return coordArray;
}

template<typename GridType, typename std::enable_if_t<!std::is_scalar<typename GridType::ValueType>::value>* = nullptr>
inline std::tuple<py::array_t<float>, py::array_t<float>>
queryValGradInt(GridType& grid, py::array_t<int32_t> coordsArray)
{
    OPENVDB_THROW(TypeError, "value and gradient query of vector grids is not yet supported. talk to @zacheng if you need this implemented.");
}
...
{
    // check input shape
    py::buffer_info info = coordsArray.request();
    if (info.ndim != 2 || info.shape[1] != 3) {
        throw py::value_error("coords should be an Nx3 array of voxel coordinates.");
    }

    const size_t N = static_cast<size_t>(info.shape[0]);
    int32_t* coordsPtr = static_cast<int32_t*>(info.ptr);

    // prepare output arrays: vals is N, grads is Nx3
    std::vector<float> vals(N, 0.0f);
    std::vector<openvdb::Vec3f> grads(N, openvdb::Vec3f(0.0f));
    ...
    for (size_t i = 0; i < N; ++i) {
        int x = coordsPtr[i * 3 + 0];
        int y = coordsPtr[i * 3 + 1];
        int z = coordsPtr[i * 3 + 2];
        openvdb::Coord ijk(x, y, z);

        // get value at the voxel coordinate
        ValueT val = accessor.getValue(ijk);

        // finite difference for gradients
        auto gradVec = openvdb::math::ISGradient<openvdb::math::DScheme::CD_2NDT>::result(accessor, ijk);

        vals[i] = static_cast<float>(val);
        grads[i] = openvdb::Vec3f(
            static_cast<float>(gradVec.x()) * 0.5,
            static_cast<float>(gradVec.y()) * 0.5,
            static_cast<float>(gradVec.z()) * 0.5
        );
    }

    // copy to numpy
    py::array_t<float> valArray({(ssize_t)N});

    {
        py::buffer_info valInfo = valArray.request();
        float* valPtr = static_cast<float*>(valInfo.ptr);
        std::memcpy(valPtr, vals.data(), N * sizeof(float));
    }


    std::vector<ssize_t> shape = { static_cast<ssize_t>(grads.size()), 3 };
    std::vector<ssize_t> strides = { 3 * static_cast<ssize_t>(sizeof(float)), static_cast<ssize_t>(sizeof(float))};
    py::buffer_info gradInfo(grads.data(), sizeof(float), py::format_descriptor<float>::format(), 2, shape, strides);
    py::array_t<float> gradArray(gradInfo);

    return std::make_tuple(valArray, gradArray);
}
...
exportGrid(py::module_ m)
{
    ...
    py::class_<GridType, GridPtr, GridBase>(m,
        ...
        .def("activeIndices", &pyGrid::activeIndices<GridType>,
            "Return all active voxel indices as a numpy array")

        .def("queryValGradInt", &pyGrid::queryValGradInt<GridType>,
            py::arg("coords"),
            "queryValGradInt(coords) -> (vals, grads)\n\n"
            "Given Nx3 voxel coordinates, return N interpolated scalar values and N gradients\n"
            "in voxel space. vals is N and grads is Nx3.")
