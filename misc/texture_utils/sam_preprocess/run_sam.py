...
# class SegMain():
#     def __init__(self, sam_path="/aigc_cfs/sz/software/seg/sam_vit_h_4b8939.pth"):
...
#         self.sam_predictor = sam_init(sam_path, gpu)
#         self.sam_predictor.model.eval()
#         print("load sam ckpt done.")


# load SAM checkpoint
sam_path = "/aigc_cfs/sz/software/seg/sam_vit_h_4b8939.pth"
gpu = os.environ.get("CUDA_VISIBLE_DEVICES", "0")
# sam_predictor = sam_init("/aigc_cfs_gdp/neoshang/models/sam/sam_vit_h_4b8939.pth", gpu)
# sam_predictor = sam_init("/aigc_cfs_2/neoshang/models/sam/sam_vit_h_4b8939.pth", gpu)
sam_predictor = sam_init(sam_path, gpu)
...
def process_image(image, bg_color, wh_ratio=0.8, save_samout=False, use_sam=False, rescale=True):
    ...
    image_processed_rgba = image_preprocess(image_seg, save_samout=save_samout, lower_contrast=False, rescale=rescale, bg_color=bg_color, wh_ratio=wh_ratio)
...
def preprocess_image(image, bg_color=255, wh_ratio=0.9, save_samout=False, use_sam=False, rescale=True):
    if isinstance(image, str):
        image = Image.open(image)
    ...
    #     rgba = repadding_rgba_image(image, rescale=True, ratio=wh_ratio, bg_color=bg_color)
    rgba = process_image(image, bg_color, wh_ratio, save_samout=save_samout, use_sam=use_sam, rescale=rescale)
...
def process_image_path(image_path, bg_color=255, wh_ratio=0.8, save_samout=False, use_sam=False, rescale=True):
    return preprocess_image(image_path, bg_color, wh_ratio, save_samout=save_samout, use_sam=use_sam, rescale=rescale)
...
if __name__ == "__main__":
    ...
    image_path = "/aigc_cfs/xibinsong/code/zero123plus_control/results/gray_18000_75_inpaint_1e_5_guidance_scale_3.5_conditioning_scale_0.75/c4e3a8fef092469294f0a1e94f1f811f/res.png"
    # image_path = "/aigc_cfs_2/neoshang/code/diffusers_triplane/data/test_quality/001_copy_4.png"
    ts = time.time()
    image = preprocess_image(image_path, wh_ratio=1.0, rescale=False)
    # image = preprocess_image(image_path)
    image.save("test_norescale.png")
    print(f"use time = {time.time() - ts}")
