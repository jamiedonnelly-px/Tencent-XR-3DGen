...
if not os.path.exists(tracer_b7_ckpt_path):
    ...
    if os.path.exists("/aigc_cfs/sz/software/seg//tracer_b7.pth"):
        shutil.copy("/aigc_cfs/sz/software/seg//tracer_b7.pth", tracer_b7_ckpt_path)
        shutil.copy("/aigc_cfs/sz/software/seg//fba_matting.pth", fba_matting_path)
    elif os.path.exists("/aigc_cfs_gdp/neoshang/models/tracer_b7/tracer_b7.pth"):
        ...
        print("ERROR mount error")
        
if not os.path.exists(u2net_path):
    print("cp u2net.onnx to cache")
    os.makedirs(os.path.dirname(u2net_path), exist_ok=True)
    local_path = "/aigc_cfs_2/neoshang/models/rembg/u2net.onnx"
    local_gdp_path = "/aigc_cfs_gdp/model/rembg/u2net.onnx"
    
    if os.path.exists(local_path):
        shutil.copy(local_path, u2net_path)
    elif os.path.exists(local_gdp_path):
        shutil.copy(local_gdp_path, u2net_path)
    else:
        print("ERROR mount error")
...
def image_preprocess(input_image, save_samout=False, lower_contrast=False, rescale=True, bg_color=255, wh_ratio=0.8):
    print("enter image_preprocess")
    image_arr = np.array(input_image)
    in_w, in_h = image_arr.shape[:2]
    if lower_contrast:
...
def remove_backgroud(images_array):
    ...
    for image in images_array:
        with torch.no_grad():
            # image_rgb = resize_image(image_rgb, 320)
...
def rescale_mask_img(image_out, origin_scale, new_scale):
...
def remove_backgroud_whitebg(images_array, rescale=False, origin_scale=0.9, new_scale=0.8):
    images_array = np.transpose(images_array, (0, 2, 3, 1))
    images_array_out = []
    for image in images_array:
        image_out = to_white_bg(image, return_rgba=True) * 255
        if rescale:
...
def to_white_bg(image, return_rgba=False):
    with torch.no_grad():
    ...
    mask[mask>0.5] = 1.0
    mask[mask<=0.5] = 0.0
    rgb[:,:,0] = rgb[:,:,0]*mask+(1-mask)
    rgb[:,:,1] = rgb[:,:,1]*mask+(1-mask)
    rgb[:,:,2] = rgb[:,:,2]*mask+(1-mask)
    rgb[rgb>1.0] = 1.0
    rgb[rgb<=0.0] = 0.0
    if return_rgba:
...
def resize_image(input_raw, size):
    ...
    return input_raw.resize((resized_w, resized_h))
