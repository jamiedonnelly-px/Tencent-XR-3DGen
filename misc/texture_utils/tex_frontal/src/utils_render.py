...
def unwrap_uv_from_vf(verts, faces, uv_res=4096, padding=2, max_stretch=0.15, parallel_regions=1):
    import open3d as o3d    # pip install open3d-cpu
    mesh = o3d.t.geometry.TriangleMesh() # if exception, pip install open3d-cpu

    mesh.vertex.positions = o3d.core.Tensor(verts.cpu().numpy(), o3d.core.float32)
    mesh.triangle.indices = o3d.core.Tensor(faces.cpu().numpy(), o3d.core.int64)
    ...
    ft = ft.reshape(faces.shape)
    vt = torch.from_numpy(vt).to(verts)
    ft = torch.from_numpy(ft).to(faces)
    return vt,ft

def unwrap_uv_from_mesh(in_mesh_path, out_mesh_path, uv_res=4096, padding=2, max_stretch=0.15, parallel_regions=1):
    ts = time.time()
    import open3d as o3d    # pip install open3d-cpu
    # mesh = o3d.t.geometry.TriangleMesh() # if exception, pip install open3d-cpu
    if not os.path.exists(in_mesh_path):
        print(f"ERROR: invalid in_mesh_path = {in_mesh_path}")
        return False
    mesh = o3d.io.read_triangle_mesh(in_mesh_path)

    mesh = o3d.t.geometry.TriangleMesh.from_legacy(mesh)

    mesh.compute_uvatlas(size=uv_res, gutter=padding, max_stretch=max_stretch, parallel_partitions=max(1,parallel_regions))

    o3d.t.io.write_triangle_mesh(out_mesh_path,mesh,)
    #    write_triangle_uvs=True)

    print('unwrap_uv_from_mesh use time ', time.time() - ts)
    return True
    ...
    if ext == ".glb":
        ...
        if not check_obj_uv(mesh_path):
            ...
            if os.path.exists(output_mesh_path):
                ...
                raise ValueError(f"uv falied to {output_mesh_path}")
    else:
...
def util_print_times(t_list):
    ...
    for i, (time, name) in enumerate(t_list):
        if i > 0:
            ...
            print(f"|||{name} use time = {t_cost}")

    total_t = t_list[-1][0] - t_list[0][0]
    print(f'||total cost time = {total_t}')
    return

if __name__ == "__main__":
    in_mesh = "/aigc_cfs_2/sz/proj/ControlNet-v1-1-nightly/data/mesh_example/difficult/obj_mesh_mesh.obj"
    # in_mesh = "/aigc_cfs_2/sz/proj/ControlNet-v1-1-nightly/data/mesh_example/obj_mesh_mesh.obj"
    out_mesh = check_mesh_uv(in_mesh)
