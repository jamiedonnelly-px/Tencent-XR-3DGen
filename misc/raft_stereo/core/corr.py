...
class PytorchAlternateCorrBlock1D:
    ...
    def corr(self, fmap1, fmap2, coords_0,coords_1):
        B, D, H, W = fmap2.shape
        # map grid coordinates to [-1,1]
        # xgrid, ygrid = coords.split([1,1], dim=-1)
        xgrid = coords_0
        ygrid = coords_1
        xgrid = 2*xgrid/(W-1) - 1  #[1, 496, 720, 9]
        ygrid = 2*ygrid/(H-1) - 1  #[1, 496, 720, 9]

        output_corr = []
        for i in range(xgrid.shape[-1]):
            # fmapw_mini = F.grid_sample(fmap2, grid_slice, align_corners=True)
            grid_slice=torch.stack([xgrid[...,i],ygrid[...,i]],axis=-1).to(coords_0.device)
            # grid_slice_np=grid_slice.data.cpu().numpy()
            # np.save("grid_slice.npy",grid_slice_np)
            fmapw_mini = bilinear_grid_sample(fmap2, grid_slice, align_corners=False)
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            centroid_lvl = coords.clone()  #[1, 496, 720, 2]
            centroid_lvl[...,0] = centroid_lvl[...,0] / 2**i
            centroid_lvl_0 = centroid_lvl[...,0].unsqueeze(3)
            centroid_lvl_1 = centroid_lvl[...,1].unsqueeze(3)
            delta_0 = delta[:,0,0]
            delta_1 = delta[:,0,1]
            coords_lvl_0 = centroid_lvl_0 + delta_0
            coords_lvl_1 = centroid_lvl_1 + delta_1
            # temp=torch.stack([coords_lvl_0,coords_lvl_1],axis=-1)
            # coords_lvl_np=temp.data.cpu().numpy()
            # np.save("coords.npy",coords_lvl_np)
            corr = self.corr(fmap1, fmap2, coords_lvl_0,coords_lvl_1)
            fmap2 = F.avg_pool2d(fmap2, [1, 2], stride=[1, 2])
            out_pyramid.append(corr)
        out = torch.cat(out_pyramid, dim=-1)
        # corr_np=corr.data.cpu().numpy()
        # np.save("corr.npy",corr_np)
