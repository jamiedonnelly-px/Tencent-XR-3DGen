...
class RAFTStereo(nn.Module):
    ...
    def forward(self, image1, image2, iters=4, flow_init=None, test_mode=True):
        ...
        image1 = (image1 / 127.5 - 1.0).contiguous()
        image2 = (image2 / 127.5 - 1.0).contiguous()
        ...
        for itr in range(iters):
            ...
            assert len(delta_flow.shape)==4
            if True:
                delta_zero = torch.zeros(delta_flow.shape[0],delta_flow.shape[1]//2,delta_flow.shape[2],delta_flow.shape[3]).to(delta_flow.device)
                delta_flow = torch.cat([delta_flow[:,0:delta_flow.shape[1]//2,:,:],delta_zero], axis=1)
            else:
                delta_zero = torch.zeros(1,1,256//8,320//8).to(delta_flow.device)
                
                delta_flow = torch.cat([delta_flow[:,0:1,:,:],delta_zero], axis=1)
            # delta_flow[:,1] = 0.0
