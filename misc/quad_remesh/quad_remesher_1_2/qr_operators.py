    ...
    if not os.path.exists(enginePath):
        ...
        try:
            ...
            if isMacOSX:
                ...
                urllib.request.urlretrieve("http://exoside.com/quadremesherdata/quad_remesher_engine_1.2_macOS.zip", zip_file_name)
            elif isLinux :
                urllib.request.urlretrieve("http://exoside.com/quadremesherdata/quad_remesher_engine_1.2_linux.zip", zip_file_name)
            else:
                urllib.request.urlretrieve("https://exoside.com/quadremesherdata/quad_remesher_engine_1.2_win.zip", zip_file_name)
...
def doRemeshing_Start(theOp, context) :
    ...
    export_path = os.path.join(QRTempFolder, "QuadRemesher", "Blender", str(uuid.uuid4()))
    ...
    try:
        ...
        ret = " ".join([enginePath, "-s", settingsFilename])
    except Exception:
        import traceback
...
class QREMESHER_OT_remesh(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        ret = doRemeshing_Start(self, context)
