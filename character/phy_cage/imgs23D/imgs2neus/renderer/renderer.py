...
from renderer.neus_networks import SDFNetwork, RenderingNetwork, SingleVarianceNetwork, PartSegmentNetwork
...
class RendererTrainer(pl.LightningModule):
    ...
    def _init_dataset(self):
        ...
        for index in range(self.num_images):
            ...
            self.img_part_info.append(np.unique(sam_singl_mask[sam_singl_mask!=-1]))
        ...
        for index in range(self.num_images):
            ...
            if len(self.img_part_info[index]) > 0:   # ignore views that only have one fg part
                for idp in self.img_part_info[index]:
                    ...
                    cur_partid_mask = torch.ones((torch.sum(cur_sam_mask).item(),1), dtype=torch.int64) * idp
        ...
        self.train_batch_num_anchors = (int(self.train_batch_num * fgpix_ratio) + self.train_batch_fg_num)
        self.train_batch_bg_num = self.train_batch_num - self.train_batch_num_anchors
    ...
    def training_step(self, batch, batch_idx):
        ...
        train_ray_batch = {k: train_ray_anc[k].cuda() for k in self.parted_ray_batch.keys()}
        ...
        loss, loss_batch = self.renderer.render_with_loss(train_ray_batch, is_train=True, step=self.global_step, num_anc=self.train_batch_num_anchors, fg_start=0)
