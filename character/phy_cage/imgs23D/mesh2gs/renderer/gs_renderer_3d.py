...
class GS3DRenderer(nn.Module):
    ...
    def initialize_(self, path):
        xyz0 = trimesh.load(os.path.join(path, "pcd_0.ply")).vertices
        xyz1 = trimesh.load(os.path.join(path, "pcd_1.ply")).vertices

        xyz = np.concatenate([xyz0, xyz1])
        num_pts = len(xyz)
        shs = np.random.random((num_pts, 3)) / 255.0
        pcd = BasicPointCloud(
            points=xyz, colors=SH2RGB(shs), normals=np.zeros((num_pts, 3))
        )
        semantics = np.zeros([len(xyz), 2])
        semantics[:len(xyz0), 0] = 1
        semantics[len(xyz0):, 1] = 1
        self.gaussians.create_from_pcd_nosem(pcd, semantics)
...
class GSRendererTrainer(pl.LightningModule):
    def __init__(self, image_path, total_steps, warm_up_steps, log_dir, train_batch_fg_num=0,
        ...
        if self.opt.load is not None:
            ...
            self.renderer.initialize_(self.xyz_init)
        self.step = 0
        # setup training
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = (1.0 - lambda_dssim) * rgb_loss + lambda_dssim * ssim_loss + self.opt.lambda_mask * mask_loss

        
        # if self.step > 3000:
        #     # regs_loss = self.renderer.gaussians.get_scaling.sum()
        #     regs_loss = (self.renderer.gaussians.get_scaling - torch.mean(self.renderer.gaussians.get_scaling.detach(), dim=-1, keepdim=True)).abs().sum()
        #     # loss += regs_loss * 1e-3
