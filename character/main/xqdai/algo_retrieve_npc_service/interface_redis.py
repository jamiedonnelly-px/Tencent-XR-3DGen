...
from animation_interface_test import  animation,animation_text,animation_text_retrieve,animation_text_retrieve_retarget
from distribute_logging import *
from shader.Run_Render import *
from is_change_ok import is_change_ok_test
import redis
...
log_folder = "/aigc_cfs_gdp/xiaqiangdai/logs/retrieveNPC_main_test"
...
class RetrieveNPCService():
    ...
    def __init__(self,host="",password='',port=6379,db_id=0,scope='webui'):
        ...
        # self.config_param = load_json(os.path.join(parent_dir,'config.json'))
        ...
        combine_client_cfg_json = os.path.join(current_file_directory,"auto_rig/layer/tdmq_interface/configs/tdmq_combine_test.json")
        self.combine_interface = CombineEasyInterface(combine_client_cfg_json)
        clothwrape_client_cfg_json =os.path.join(current_file_directory,'clothwrap2/tdmq_interface/configs/tdmq_test.json')
    ...
    def exposed_hair_is_change_ok(self, mesh_path):
        ...
        """
        try:
            ret = is_change_ok_test(mesh_path)
        except Exception as e:
