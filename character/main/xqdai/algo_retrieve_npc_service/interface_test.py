...
from single_retrieve_test import retrive_single_txt
import uuid
import time
import json
import requests
...
if not os.path.exists("/root/envs/auto_rig"):
    ...
    os.system("mkdir /root/envs/ && cp -r /mnt/aigc_cfs_cq/xiaqiangdai/envs/auto_rig.tar.gz /root/envs/")
...
class RPCService(Service):
    ...
    def __init__(self):
        ...
        self.log_file_path = f"/aigc_cfs_2/xiaqiangdai/data/rpc_logs/{year}_{month}_{day}.log"
    ...
    def on_connect(self, conn):
        ...
        if self.processNum>100:
            remove_files_older_than_n_days("/aigc_cfs_2/xiaqiangdai/data/rpc_logs",10)
            now = datetime.now()
            year = now.year
            month = now.month
            day = now.day
    ...
    def wrap_cloth(self,mesh_output_path, paths_temp):
        ...
        self.logger.info(f"{self.uuid} wrap_cloth {paths_temp}")
        ...
        cmd = [
            ...
            "/aigc_cfs_2/xiaqiangdai/project/cloth_wrap/webui/cloth_warpper_test.py",
            "--lst_path",
            obj_lst,
        ]
    ...
    def exposed_cos_upload(self,glb_local_path):
        ...
        remote_file_path = os.path.join(cos_client.retrieve_NPC_dir_remote, new_glb_name)
    ...
    def exposed_txt_retrive_npc(self, entity,description):
        ...
        if gender not in genders:
            self.logger.error(f"{self.uuid} txt_retrive_npc gender error")
        ...
        final_out.append(entity[-1]) #gender 18
        final_out.append(suit_enale)# 19
        if keys_out[0]!=None:
        ...
        end_time = time.time()
        self.logger.info(f"{self.uuid} txt_retrive cost time: {end_time - start_time} s")
    ...
    def exposed_long_retrive_npc_all_auto(self,long_prompt):
        ...
        if res.status_code == 200:
            ...
            self.logger.error(f"{self.uuid} autoRig_layer auto_rig Request failed with status code")
            return None, None, []
        ...
        final_out= [os.path.join(mesh_output_path, "mesh/mesh.glb"),os.path.join(mesh_output_path, "mesh/mesh_animation.fbx"),gif_path]
        json_str = ujson.dumps(final_out)
    ...
    def exposed_retrive_npc_all_auto(
        ...
        paths_temp_in = {"path": paths_temp, "body_attr": [gender, shape],"hair_color":hair_color}
        self.logger.info(f"{self.uuid} paths_temp_in:{paths_temp_in}")
        ...
        if res.status_code == 200:
            ...
            self.logger.erroe(f"{self.uuid} app_autoRig_layer combine Request failed with status code")
            return None, None, []
    ...
    def exposed_retrive_npc_combine(
        ...
        paths_temp = {}
        attr_keys = ["hair", "top", "trousers", "shoe", "outfit", "others"]
        self.logger.info(f"{self.uuid} path_list:{path_list}  key_list:{key_list}  texture_replace:{texture_replace}")
        global g_json_data
        for i, key in enumerate(key_list):
            if key != None and key != "" and key != " " and texture_replace[i] == False:
                paths_temp[g_json_data[key]["Obj_Mesh"]] = {
        ...
        if not os.path.exists(mesh_output_path):
            os.makedirs(mesh_output_path)
            self.logger.info(f"{self.uuid} {mesh_output_path} not exists,make")
        else:
            self.logger.info(f"{self.uuid} {mesh_output_path}  exists")
        ...
        # time.sleep(20)
        # return ('/mnt/aigc_cfs_cq/xiaqiangdai/project/objaverse_retrieve/data/252c2813-6d75-5f2b-8faf-68cb5cd371ba',"/mnt/aigc_cfs_cq/xiaqiangdai/project/objaverse_retrieve/data/252c2813-6d75-5f2b-8faf-68cb5cd371ba/mesh/c32e29ee6d554d2a8983e5c0abeef17e.glb")
    ...
    def exposed_retrive_npc_text_animation(self, mesh_output_path,text_prompt):
        ...
        fbx_path = os.path.join(mesh_output_path, "mesh/mesh.fbx")
        self.logger.info(f"{self.uuid} mesh_output_path:{mesh_output_path} text_prompt:{text_prompt}")
        try:
    ...
    def exposed_auto_rig_manual_render(self, mesh_out_path):
        ...
        if res.status_code == 200:
            ...
            self.logger.error(f"{self.uuid} app_autoRig auto_rig failed with status code")
            return None
    ...
    def exposed_auto_rig_manual_calculate(self, joints_data, mesh_out_path):
        ...
        if res.status_code == 200:
            ...
            self.logger.info(f"{self.uuid} app_autoRig auto_rig failed with status code")
            return None
    ...
    def exposed_available(self, type="generateCharacter"):
    ...
    def exposed_render_shader(self, model_path):
        ...
        model_folder = os.path.dirname(model_path)
        render_foler  = os.path.join(model_folder,'render')
        blenderFile_path = "/aigc_cfs_2/xiaqiangdai/project/algo_retrieve_npc_service/shader/RenderFactory.blend"
    ...
    def exposed_shape_text_retrieve(self,gender, shape_promt):
        """
        shape_text_retrieve
        ...
        final_out = {"gender":gender,"shape":body_attr[1],"use_shoes":use_shoes,"use_hair":use_hair}
        json_str = ujson.dumps(final_out)
...
if __name__ == "__main__":
    ...
    server1 = ThreadPoolServer(RPCService, hostname=args.hostname1, port=80,nbThreads=128)
