...
def calc_embedding(model,processor,args,keys):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--outdir", default='/mnt/aigc_cfs_cq/xiaqiangdai/project/retrieve_lantent/normic_lantent')
    ...
    pool_list = []
    if thread_num==1:
        calc_embedding(model,processor,args,keys)
    else:
        for i in range(thread_num):
            if i!=thread_num-1:
                input_arg = (model,processor,args,keys[num_all//thread_num*i:num_all//thread_num*(i+1)])
            else:
                input_arg = (model,processor,args,keys[num_all//thread_num*i:num_all])
