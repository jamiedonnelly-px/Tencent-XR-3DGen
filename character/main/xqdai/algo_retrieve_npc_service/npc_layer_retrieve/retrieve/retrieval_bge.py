...
def calc_txt_emb(text, text_model,device):
    with torch.no_grad():
        text_embeddings = text_model.encode(text=text)
...
if __name__ == "__main__":
    ...
    with open('/mnt/aigc_cfs_cq/xiaqiangdai/project/image_retrieve/lib_generate/test_list.json', 'r') as file:
        list_test = json.load(file)

    with open('/mnt/aigc_cfs_cq/xiaqiangdai/project/image_retrieve/lib_generate/caption_hunyuan_en.json', 'r') as file:
    ...
    print(f"top5_num:{top5_num} all_num:{len(list_test)}")
