...
def calc_embedding(model,preprocess,args,keys):
    cnt = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--json_path", default='/mnt/aigc_cfs_cq/xiaqiangdai/project/objaverse_retrieve/20240709_ruku_ok.json')
    parser.add_argument("--outdir", default='/mnt/aigc_cfs_cq/xiaqiangdai/project/retrieve_lantent/clip_lantent')
    ...
    if thread_num==1:
        calc_embedding(model,preprocess,args,keys)
    else:
        for i in range(thread_num):
            if i!=thread_num-1:
                config.append((model,preprocess,args,keys[num_all//thread_num*i:num_all//thread_num*(i+1)]))
            else:
                config.append((model,preprocess,args,keys[num_all//thread_num*i:num_all]))
        
        mp.set_start_method('spawn')
        with multiprocessing.Pool() as pool:
            async_result = pool.starmap_async(calc_embedding, config)
