...
IMPORTER_CLASS = [BetterImportFbx]

def importFBXFile(filepath):
    #filepath = 'E:\\blender\\20240416\\KOF ALLSTAR拳皇人物\\01_MayLee_Lobby\\01_MayLee_Lobby\\01_MayLee_Lobby.fbx'
    ...
    result = subprocess.run([executable_path, filepath, output_path, "1.0", "None", "None", "None", "False", "True", "None", "None", "True", "True", "True", "False", "None","False", 'FBXSDK', "None", "None", "None", "None", "None", 'cm', "None", "None", "False"])
    if result.returncode != 0:
        if os.path.exists(output_path):
            os.remove(output_path)
        return {'CANCELLED'}
    ...
    result = read_some_data(context, output_path, my_leaf_bone, my_import_normal, my_shade_mode, use_auto_smooth, my_angle, use_auto_bone_orientation, my_bone_length, my_calculate_roll, use_vertex_animation, use_edge_crease, my_edge_crease_scale, my_edge_smoothing, use_import_materials,  None, my_rotation_mode, use_detect_deform_bone, use_fix_bone_poses, use_attach_to_selected_armature, my_animation_offset, use_animation_prefix, primary_bone_axis, secondary_bone_axis, use_pivot)
    if result != {'FINISHED'}:
        if os.path.exists(output_path):
            os.remove(output_path)
        return {'CANCELLED'}
...
blenderFile_path = r"./RenderFactory.blend"

origin_offset = 0.4

def fix_Camera_location(center, size):
    default_len = 2.3350733344468826
    camera_default_height = 1.4490749835968018
    default_height = 0.5602275282144547
    camera_name = bpy.data.cameras[0].name
    camera = bpy.data.objects[camera_name]
    half_bounds_height = size[2]/2
    print("Center: {0}".format(center))

    # 首先先将高度对其到center
    camera.location[2] += half_bounds_height - default_height
    print("Move Height: {0}".format(half_bounds_height - default_height))

    # 计算需要移动的位置
    forward_dir = camera.matrix_local @ (Vector((1, 0, 0)).normalized())
    move_len = ((default_len * half_bounds_height) / default_height) - default_len
    camera.location += forward_dir * move_len
    print("Move dir: {0} move len: {1}, now len : {2}".format(forward_dir, move_len, ((default_len * half_bounds_height) / default_height)))
    camera.location[0] = 0

    return

def scale_model(root_mesh, center, size, bounds_min):
    default_center = Vector( (-8.41319561e-04 ,3.53442200e-01 ,9.08355102e-01) )
    default_height = 1.120455060
    root_mesh.location += Vector((default_center[0] - center[0],default_center[1] - center[1], 0))

    # 修改代码中的1.8为最小的缩小基数
    tmp_size = size * default_height/size[2]
    tmp_len = math.sqrt(tmp_size[0]*tmp_size[0] + tmp_size[1]*tmp_size[1])
    tmp_scale =  1.3/tmp_len if 1.3/tmp_len < 1 else 1

    # 原本中心到最小值的长度
    half_z = root_mesh.location[2] - bounds_min[2]

    scale_size_z = half_z - (half_z*root_mesh.scale[2] * (default_height/size[2]) * tmp_scale)
    
    #(1/math.tan(bpy.data.objects['Camera'].rotation_euler.x)) * bpy.data.objects['Camera'].location[2]-default_height
    
    move_z = (bounds_min[2] + scale_size_z)
    move_y = (1/math.tan(bpy.data.objects['Camera'].rotation_euler.x)) * move_z
    root_mesh.location[2] -= move_z
    root_mesh.location[1] += move_y
    #root_mesh.location[2] -= (bounds_min[2] * root_mesh.scale[2])

    root_mesh.scale *= (default_height/size[2]) * tmp_scale

    print("Center : {}".format(center))
    print("Size : {}".format(size))
    print("bound_min : {}".format(bounds_min))
    return

def set_shader_smooth(obj):
    polygons = obj.data.polygons
    polygons.foreach_set('use_smooth',  [True] * len(polygons))
    obj.data.update()


def set_model_loc_scale_keyframe(obj, center, bounds_exent, bounds_min):
    obj.keyframe_insert(data_path="location",frame=1,index=1)
    obj.keyframe_insert(data_path="scale",frame=1,index=1)
    obj.keyframe_delete(data_path = "location")
    obj.keyframe_delete(data_path = "scale")
    
    scale_model(obj, center, bounds_exent, bounds_min)
    bpy.ops.object.transform_apply(location=True, rotation=True, scale=True)

def move_min_z():
    root_obj = None if len(bpy.context.selected_objects) != 1 else bpy.context.selected_objects[0]
    points = []
    ...
    for obj in bpy.context.selected_objects:
        if obj.type == 'ARMATURE':
            ...
            for point in obj.bound_box:
                new_point = mat @ Vector((point[0], point[1], point[2]))
                points.append([new_point[0], new_point[1], new_point[2]])
    ...
    if root_obj == None:
        ...
        for obj in bpy.context.selected_objects:
            if obj.type == "MESH":
                set_model_loc_scale_keyframe(obj, center, bounds_max - bounds_min, bounds_min)
                # bpy.context.scene.cursor.location = Vector( (-8.41319561e-04 ,3.53442200e-01 - origin_offset,9.08355102e-01) )
                # bpy.ops.object.origin_set(type='ORIGIN_CURSOR', center='MEDIAN')
                obj.rotation_mode = 'XYZ'
                obj.rotation_euler[2] = 0.261799
                # cosAngle = math.cos((15/360)*6.2831853)
                # sinAngle = math.sin((15/360)*6.2831853)
                # mat = obj.matrix_world
                # original = mat @ obj.location
                # original_to_center = Vector((center[0] - original[0], center[1] - original[1], 0))
                # new_center = Vector((-sinAngle*original_to_center[1] + cosAngle * original_to_center[0], cosAngle * original_to_center[1] + sinAngle * sinAngle * original_to_center[0], 0))
                # obj.location = obj.location + (original_to_center - new_center)
    else:
        #root_obj.original.location = center
        set_model_loc_scale_keyframe(root_obj, center, bounds_max - bounds_min, bounds_min)
        bpy.context.scene.cursor.location = Vector( (-8.41319561e-04 ,3.53442200e-01 - origin_offset ,9.08355102e-01) )
        bpy.ops.object.origin_set(type='ORIGIN_CURSOR', center='MEDIAN')
        obj.rotation_mode = 'XYZ'
        root_obj.rotation_euler[2] = random.choice([0.261799, -0.261799])

def fixMaterials(material_array):
    for obj in bpy.context.selected_objects:
        if obj.type == "MESH":
            for slot in obj.material_slots:
                if type(slot.material) != bpy.types.Material: continue
                if slot.material not in material_array:
                    material_array.append(slot.material)
                    BSDF = slot.material.node_tree.nodes['Principled BSDF']
                    if (len(BSDF.inputs['Alpha'].links) > 0):
                        Alpha_link = BSDF.inputs['Alpha'].links[0]
                        slot.material.node_tree.links.remove(Alpha_link)

                    BSDF.inputs['Alpha'].default_value = 1
                    BSDF.inputs['Emission Strength'].default_value = 0
                    if BSDF.inputs['Roughness'].default_value > 1:
                        BSDF.inputs['Roughness'].default_value = 1

                    add_toon_material(slot.material.node_tree, BSDF)

def add_random_light(collect):
    light_num = random.randint(2, 5)
    plane = random.choice(["RandomPlane1", "RandomPlane2"])
    ...
    for i in range(0, light_num):
        ...
        light_random_x = random.uniform(-2.32673, 2.32673)
        light_random_y = random.uniform(0, 4.41077063079834)
        light.location = Vector((-0.0017219999572262168 + light_random_x, 2.59468936920166 + light_random_y, 2.3194661140441895))
        light_random_x = random.uniform(-0.6981317, 0.6981317)
        light_random_y = random.uniform(-1.0472, 1.0472)
        light_random_z = random.uniform(0, 3.1415926)
        light.rotation_euler = [light_random_x, light_random_y, light_random_z]

        max_len = 8
        if(light_random_x > 0.52359877):
            max_len = 3
        scale_size = random.uniform(1, max_len)
        light.data.energy = random.uniform(7.5, 15)
        if(abs((light_random_z*360/ (6.283185)) - 180) < 45):
            light.scale = [1, scale_size, 1]
        else:
            light.scale = [scale_size, 1, 1]
            light.location[2] += 1
            light.data.energy -= 1
        collect.objects.link(light)
        bpy.context.scene.collection.objects.unlink(light)


def add_toon_material(node_tree, BSDF):
    toon = node_tree.nodes.new(type="ShaderNodeBsdfToon")
    if (len(BSDF.inputs['Base Color'].links) <= 0):
        ...
        node_tree.links.new(toon.inputs["Color"], color)
    toon.inputs['Size'].default_value = 0.82

def set_toon_material(material_array):
    for material in material_array:
        toon = material.node_tree.nodes['Toon BSDF']
        output = material.node_tree.nodes['Material Output'].inputs['Surface']
        material.node_tree.links.new(toon.outputs["BSDF"], output)

def reset_toon_material(material_array):
    for material in material_array:
        Principled = material.node_tree.nodes['Principled BSDF']
        output = material.node_tree.nodes['Material Output'].inputs['Surface']
        material.node_tree.links.new(Principled.outputs["BSDF"], output)

def add_dissolve_material(material_array):
    shaderMaskNode = bpy.data.node_groups['shaderMask']
    for material in material_array:
        Principled = material.node_tree.nodes['Principled BSDF']
        #add the group
        dummy_group_node = material.node_tree.nodes.new(type='ShaderNodeGroup')
        dummy_group_node.node_tree = shaderMaskNode

        material.node_tree.links.new(Principled.inputs['Alpha'], dummy_group_node.outputs['mask'])
        material.node_tree.links.new(Principled.inputs['Emission Strength'], dummy_group_node.outputs['center'])
        material.node_tree.links.new(Principled.inputs['Emission'], dummy_group_node.outputs['Color'])

def render_toon_style(material_array, out_iamge_path, name):
    set_toon_material(material_array)
    bpy.context.scene.render.filepath = out_iamge_path + "\\" + name +  "toon.png"
    bpy.ops.render.render(write_still = True)
    reset_toon_material(material_array)

def render_freeStyle(out_iamge_path, name):
    bpy.context.scene.render.use_freestyle = True
    bpy.context.scene.render.filepath = out_iamge_path + "\\" + name + "freeStyle.png"
    bpy.ops.render.render(write_still = True)
    bpy.context.scene.render.use_freestyle = False

def render_collect(material_array, out_iamge_path):
    for collection in bpy.data.collections:
        collection.hide_render = False
        #move_min_z()
        if collection.name == "CoverCollect":
            print("#################### CoverCollect ##################################")
            add_random_light(collection)
            bpy.context.scene.view_settings.look = 'None'
            bpy.data.objects["Area.020"].data.color = (1, 1, 1)
            bpy.data.objects["Area.020"].data.energy = 400
        elif collection.name == "Collection 6" or collection.name == "Collection 2.001":
            bpy.context.scene.view_settings.look = 'None'
            bpy.data.objects["Area.020"].data.energy = 400
        ...
        bpy.context.scene.render.filepath = out_iamge_path + "\\" + collection.name + ".png"
        bpy.ops.render.render(write_still = True)
        bpy.context.scene.view_settings.look = 'Very High Contrast'
        bpy.data.objects["Area.020"].data.color = (1.0, 0.8430169820785522, 0.6359171867370605)
        bpy.data.objects["Area.020"].data.energy = 300

        ## 添加风格化渲染
        # render_freeStyle(out_iamge_path, collection.name)
        # render_toon_style(material_array, out_iamge_path, collection.name)
...
def render_ChineseStyle(out_iamge_path):
    chinese = bpy.data.materials['chinese']
    for obj in bpy.context.selected_objects:
        if obj.type == "MESH":
            for slot in obj.material_slots:
                slot.material = chinese
    bpy.context.scene.render.filepath = out_iamge_path + "\\" + "chineseStyle.png"
    bpy.ops.render.render(write_still = True)

def render_dissolve(material_array, out_iamge_path):
    # 添加溶解材质
    add_dissolve_material(material_array)

    for i in range(0, 120, 3):
        bpy.context.scene.frame_set(i)

        bpy.context.scene.render.filepath = out_iamge_path + "\\" + "dissolve\\dissolve" + str(i) + '.png'
        bpy.ops.render.render(write_still = True)
...
def waitForAddMesh():
    ...
    # 调整大小，以及位置
    move_min_z()
    # 修复默认导入fbx后，产生的自发光和遮罩
    fixMaterials(material_array)
    out_iamge_path =os.path.join(os.path.dirname(fbx_path),'render')
    if not os.path.isdir(out_iamge_path):
        os.makedirs(out_iamge_path)
    ## #
    #  渲染灯光
    ##
    render_collect(material_array, out_iamge_path)
    bpy.data.objects['Sun'].hide_render = False
    ## #
    #  渲染描边
    ##
    #render_freeStyle(out_iamge_path, "")
    ## #
    #  渲染卡通风格
    ##
    #render_toon_style(material_array, out_iamge_path, "")
    ## #
    #  渲染水墨风
    ##
    #render_ChineseStyle(out_iamge_path)
    ## #
    #  渲染溶解
    ##
    #render_dissolve(material_array, out_iamge_path)
...
def RenderMesh(x, y):
    ...
    # 打开用于渲染的blender文件
    #bpy.ops.wm.open_mainfile(filepath=blenderFile_path)
    #bpy.ops.object.select_all(action="DESELECT")
    # while(not hasattr(bpy.context, "selected_objects")):
    #     print("####################### ERROR #################################")
    # 加载需要导入的模型
    ...
    # 处理材质
    # 这里需要延迟处理，不然不知道为什么blender会读取不到context
    #bpy.app.timers.register(functools.partial(waitForAddMesh), first_interval=0.5)
    waitForAddMesh()
