...
from tdmq_client_combine import CombineBackendConsumer, CombineProducer

def init_job_id():
    return str(uuid.uuid4())

class CombineEasyInterface:
    def __init__(
        self,
        client_cfg_json=os.path.join(codedir, 'configs/tdmq_combine.json'),
    ):
        self.service_name = "combine"
        self.backend_consumer = CombineBackendConsumer(client_cfg_json)
        consumer_thread = threading.Thread(target=self.backend_consumer.start_consumer)
        consumer_thread.start()
        time.sleep(0.1)
        logging.info("run CombineBackendConsumer done")

        self.producer = CombineProducer(client_cfg_json)
    ...
    def blocking_call_combine(self, job_id, in_mesh_path, output_mesh_filename=None, timeout=300):
        ...
        Returns:
            ...
            result_meshs list of obj path
        """        
        logging.info(f"blocking_call_combine begin {job_id}")
        query_func = lambda: self.producer.interface_tdmq_combine(
            job_id=job_id, in_mesh_path=in_mesh_path, output_mesh_filename=output_mesh_filename
        )
        logging.info(f"blocking_call_combine end {job_id}")
    ...
    def parse_final_result_dict(self, job_id, final_result_dict):
        ...
        """
        try:
            value_dict = final_result_dict[job_id]["combine"]
...
def run_blocking_call_combine(args):
    combine_interface, job_id, in_mesh_path, output_mesh_filename, timeout = args
    return combine_interface.blocking_call_combine(job_id,
                                                     in_mesh_path,
...
if __name__== '__main__':
    ...
    parser.add_argument('--client_cfg_json',
                        type=str,
                        default=os.path.join(codedir, 'configs/tdmq_combine.json'),
                        help='relative name in codedir/configs')
    args = parser.parse_args()
    ### prepare example data

    in_mesh_path = "/aigc_cfs_gdp/xiaqiangdai/retrieveNPC_save/52ad08f5-6f4f-5c76-9c14-59ee55113f36"
    out_objs_filename = os.path.join(in_mesh_path, "/mesh/mesh.glb")
    job_id = init_job_id()

    # 1. init
    combine_interface = CombineEasyInterface(args.client_cfg_json)

    # # 2. 堵塞式调用, 类似grpc
    success_flag, result_meshs = combine_interface.blocking_call_combine(job_id,
                                                            in_mesh_path,
                                                            output_mesh_filename=os.path.join(in_mesh_path, "mesh/mesh.glb"))
    assert success_flag
    combine_interface.close()
