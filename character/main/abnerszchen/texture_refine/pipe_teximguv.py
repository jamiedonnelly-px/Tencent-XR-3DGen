...
from render.util import mse_to_psnr, save_image
...
class ObjTexImgUVPipeline:
    def __init__(self, in_model_path, in_sd_path, ip_adapter_model_path, device="cuda"):
        """End-End object texture text-img UV control pipeline class
        ...
        try:
            ...
            raise ValueError("init ObjTexImgUVPipeline failed")

    def init_model_glctx(
        self, in_model_path, in_sd_path, ip_adapter_model_path, seed=42
    ):
    ...
    def load_geom(self, normal_path):
    ...
    def infer_img_uv(
        ...
        Args:
            ...
            in_uv_pos_pils (list of pil): list of normal pil(0-255, h, w, 3). TODO(csz) need support list of str/pil/tensor/np
            in_uv_normal_pils (list of pil)
        ...
        """
        if len(in_uv_pos_pils) != len(in_prompts) or len(in_uv_pos_pils) != len(
            in_uv_normal_pils
        ):
            print(
        ...
        pipeline: SDControlNetUVPipeline = self.pipeline
        generator: torch.Generator = self.generator

        if in_condi_img_pils is not None:
            if len(in_uv_pos_pils) != len(in_condi_img_pils):
                print(
                    "ERROR invalid input, not equal list ",
                    len(in_uv_pos_pils),
                    len(in_condi_img_pils),
                )
                return
    ...
    def obj_teximguv(
        ...
        """for one obj, render UV normal, generate uv texture maps with in_prompts
        ...
        Returns:
            new_meshs: list of new mesh : list of class Mesh
        """

        # 1. load obj and render uv pos & normal
        ...
        # only support one obj, one condition img with multi text now, can not with multi-condition img
        in_condi_img_pils = None
        ...
        raw_mesh: Mesh = load_mesh(in_obj, skip_mtl=True)
        ...
        gb_pos, gb_normal, gb_mask = render_geometry_uv(self.glctx, raw_mesh, uv_res)
        pos_pil = cvt_geom_to_pil(gb_pos, gb_mask)
        normal_pil = cvt_geom_to_pil(gb_normal, gb_mask)

        # 2. infer  model, get uv texture
        in_uv_pos_pils = [pos_pil] * len(in_prompts)
        in_uv_normal_pils = [normal_pil] * len(in_prompts)
        out_uv_pils = self.infer_img_uv(
            in_prompts,
            in_uv_pos_pils,
            in_uv_normal_pils,
            in_condi_img_pils,
        ...
        if not out_uv_pils:
            print("ERROR infer_img_uv failed")
            return None
        ...
        for i, out_uv_pil in enumerate(out_uv_pils):
            ...
            image = (
                torch.from_numpy(np.array(out_uv_pil)).unsqueeze(0) / 255.0
            )  # [1,h,w,3]
        ...
        try:
            if out_debug_dir is not None:
                ...
                vis_pil = [in_uv_pos_pils, in_uv_normal_pils]
                if in_condi_img_pils is not None and len(in_condi_img_pils) == len(
                    in_uv_pos_pils
                ):
                    vis_pil.append(in_condi_img_pils)
                vis_pil.append(out_uv_pils)
                concatenate_images_2d(
    ...
    # interface for grpc
    def interface_obj_teximguv(
        self, in_obj, out_objs_dir, in_prompts=None, in_condi_img=None, run_cfg=None
    ):
        """grpc interface: get uv texture from obj, text and image
    ...
    # unit test
    def test_pipe_obj_teximguv(
        self, in_obj, out_objs_dir, in_prompts=None, in_condi_img=None, debug_save=True
    ):
        run_cfg = self.cfg
        run_cfg.debug_save = debug_save
        print("input test_pipe_obj_teximguv: ", in_obj, in_prompts, out_objs_dir, in_condi_img)
        out_obj_paths = self.interface_obj_teximguv(
            in_obj,
            out_objs_dir,
            in_prompts=in_prompts,
            in_condi_img=in_condi_img,
