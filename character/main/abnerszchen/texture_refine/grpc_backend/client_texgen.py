...
def query_mesh_from_key(web_flatten_dict, in_mesh_path, in_mesh_key):
    ...
    try:
        ...
        if os.path.exists(Mesh_obj_raw):
            print(f"debug find valid mesh {Mesh_obj_raw } from in_mesh_key {in_mesh_key}")
            return Mesh_obj_raw
        else:
...
# main class
class TexGenClient():
    def __init__(self, client_cfg_json='client_texgen.json', model_name='uv_mcwy', device='cuda'):
        """client
        ...
        try:
            self.web_flatten_json = self.run_cfg.web_flatten_json
            assert os.path.exists(self.web_flatten_json), self.web_flatten_json
            self.web_flatten_dict = load_json(self.web_flatten_json)
            print(f"load web_flatten_json done from {self.web_flatten_json}, find {len(self.web_flatten_dict)} objs")
    ...
    def client_obj_tex_gen(
        ...
        in_mesh_path = query_mesh_from_key(self.web_flatten_dict, in_mesh_path, in_mesh_key)
        logging.info(
        ...
        with grpc.insecure_channel(server_addr) as channel:
            ...
            print(f'[Succeed], job_id {job_id} get out_mesh_paths_queryed:{out_mesh_paths_queryed}')
...
def test_client(client_cfg_json, model_key):
    ...
    if model_key == 'control_ready':
        ...
        in_mesh_path = f"/aigc_cfs_gdp/cfs1/Asset/designcenter/clothes/convert/mcwy2/remove_skin_mesh/meshes/Top/BR_TOP_1_F_T/BR_TOP_1_fbx2020.obj"
        out_objs_dir = f"/aigc_cfs_gdp/cfs1/sz/server/tex_gen/client_log/mcwy_debug_gdp_{model_key}"
        # in_mesh_path = f"/aigc_cfs/Asset/designcenter/clothes/convert/mcwy2/remove_skin_mesh/meshes/Top/BR_TOP_1_F_T/BR_TOP_1_fbx2020.obj"
        # in_mesh_path = f"/aigc_cfs/layer_avatar_data/mcwy_2/objs_three/MCWY_2_Dress/Dresses_F_A_DR_640_F_A_DR_640_fbx2020_output_512_MightyWSB/uv_condition/mesh.obj"
        # out_objs_dir = f"/aigc_cfs_3/sz/server/tex_gen/client_log/mcwy_debug_{model_key}"
        # in_prompts = ""
        in_condi_img = "/aigc_cfs/Asset/designcenter/clothes/render_part2/render_data/dress/render_data/DR_640_F_A/Dresses_F_A_DR_640_F_A_DR_640_fbx2020_output_512_MightyWSB/color/cam-0032.png"
        in_prompts = "indian style"
        # in_prompts = "red chinese dragon"
        ...
        in_mesh_path = "/aigc_cfs/sz/data/tex/lowpoly/low_poly/Dog_C_Police/uv_condition/mesh.obj"
    ...
    print(f"external call example: job_id {job_id} job_status {job_status}, run_cnt {run_cnt}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_key', type=str, default='uv_mcwy', help='select model. can be uv_mcwy, control_ready, control_mcwy, imguv_mcwy, imguv_lowpoly')
