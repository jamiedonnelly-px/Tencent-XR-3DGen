...
if [ ${train_cnt} = "300" ]; then
    ...
    validation_epochs=2
elif [ ${train_cnt}  = "all" ]; then
    export DATASET_JSON="/aigc_cfs_3/sz/data/tex/human/design_lowpoly_vroid/design_only_withtext/tex_creator_train.json"
    export DATASET_VAL_JSON="/aigc_cfs_3/sz/data/tex/human/design_lowpoly_vroid/design_only_withtext/tex_creator_val.json"
    export DATASET_TEST_JSON="/aigc_cfs_3/sz/data/tex/human/design_lowpoly_vroid/design_only_withtext/tex_creator_test.json"
...
output_dir=/aigc_cfs_3/sz/result/tex_creator/human/pose9_argum/g4_text/design_only_${train_cnt}_b${train_batch_size}a${gradient_accumulation_steps}_ns${noise_scheduler}
mkdir -p ${output_dir}
