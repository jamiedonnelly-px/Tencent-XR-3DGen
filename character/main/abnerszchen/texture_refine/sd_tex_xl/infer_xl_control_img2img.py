...
def infer_enum_batch(
    ...
    for i in range(0, len(prompt_texts), batch_size):
        ...
        num_inference_steps_l = [20, 50]  # [10, 20, 50]
        strength_l = [0.99, 1]
        guidance_scale_l = [3., 7.5, 9.0]  # [0.5, 1, 1.5, 2, 3, 5, 7]
        controlnet_conditioning_scale_l = [0.6, 0.8]
        # guidance_scale_l = [4, 5, 7.5, 10]  # [0.5, 1, 1.5, 2, 3, 5, 7]

        for num_inference_steps in num_inference_steps_l:
            for strength in strength_l:
                for guidance_scale in guidance_scale_l:
                    for controlnet_conditioning_scale in controlnet_conditioning_scale_l:
                        ...
                        edited_images = pipeline(
                            prompt=prompt_text,
                            image=gt_pils, # sd_images, #gt_pils,
                            control_image=condi_pils,
                            strength=strength,
                        ...
                        output_image = concatenate_images_2d(
                            ...
                            os.path.join(
                                out_dir,
                                f"b_{i}_n_{num_inference_steps}_s{strength}_gs_{guidance_scale}_ccs_{controlnet_conditioning_scale}.jpg",
                            ),
                        )
