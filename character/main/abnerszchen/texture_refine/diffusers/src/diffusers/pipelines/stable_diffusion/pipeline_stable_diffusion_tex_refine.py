...
class StableDiffusionTexRefinePipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    ...
    def __call__(
        self,
        prompt: Union[str, List[str], PipelineImageInput] = None,
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                print('debug latents ', latents.shape, self.do_classifier_free_guidance)
                print('debug prompt_embeds ', prompt_embeds.shape)
                ...
                print('debug unet test ', scaled_latent_model_input.shape, t, prompt_embeds.shape)
    ...
    def _encode_prompt(
        ...
        if is_prompt_image:
            ...
            if prompt is not None and isinstance(prompt, str):
                image = PIL.Image.open(prompt).convert("RGB").resize((224, 224))
            elif prompt is not None and isinstance(prompt, list):
                image = [PIL.Image.open(img).convert("RGB").resize((224, 224)) for img in  prompt]
