...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_mesh_name', type=str, default="combined.obj",
                        help='name of output obj mesh file')
    parser.add_argument('--out_res', type=int, default=512,
    ...
    output_mesh_name = args.output_mesh_name
    ...
    obj_mesh_file = os.path.join(output_mesh_folder, output_mesh_name)
    ...
    try:
        merge_tex_path = os.path.join(os.path.dirname(obj_mesh_file), "Atlas_00001.png")
        image = Image.open(merge_tex_path)

        assert image.size[0] == image.size[1], image.size
        if image.size != (out_res, out_res):
            image = image.resize((out_res, out_res), Image.LANCZOS)

        bak_path = os.path.join(os.path.dirname(obj_mesh_file), "Atlas_00001.bak.png")
        shutil.copyfile(merge_tex_path, bak_path)
        image.save(merge_tex_path)
        
        print("Combine obj file from %s and export combined obj to %s" %
            (input_mesh_path, obj_mesh_file))
    
    except Exception as e:
        print('process texture map failed', e)
        shutil.move(obj_mesh_file, obj_mesh_file.replace('combined.obj', 'merge_but_failed.obj'))
