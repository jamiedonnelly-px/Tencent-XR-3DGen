...
def infer_batch(pipeline,
    ...
    Args:
        ...
        uv_pos_paths: list of in uv position image path
        uv_normal_paths: list of in uv normal image path
        condi_img_paths: list of in condition image path
    ...
    for i in range(0, in_cnt, batch_size):
        ...
        vis_pils_2d = [normal_pils, condi_img_pils, edited_images, gt_pils]
        if infer_sd:
...
def make_infer_data(objs_dict, sample_key_pair_list, use_obj_all=False, sample_render_img=False):
    ...
    Returns:
        prompt_texts, uv_pos_paths, uv_normal_paths, condi_img_paths, uv_kd_paths, (gt_render_imgs can be []), list of str or path
    """
    # run inference
    prompt_texts, uv_pos_paths, uv_normal_paths, condi_img_paths, uv_kd_paths, gt_render_imgs = [], [], [], [], [], []
    ...
    for d, dname, oname in sample_key_pair_list:
        ...
        condi_imgs = meta_dict['condi_imgs_in']
        cap_cnt, condi_img_cnt = len(caption_list), len(condi_imgs)
        # TODO if run test, can be empty
        if cap_cnt == 0 or condi_img_cnt ==0:
            continue

        if not use_obj_all:
            caption_list = [caption_list[random.randint(0, cap_cnt - 1)]]
            cap_cnt = 1
            condi_imgs = [condi_imgs[random.randint(0, condi_img_cnt - 1)]]
            condi_img_cnt = 1
            
        print('caption_list', caption_list)
        print('condi_imgs', condi_imgs)

        if sample_render_img and 'render_imgs' in meta_dict:
            render_imgs = meta_dict['render_imgs']
            img_cnt = len(render_imgs)
            indices = random.sample(range(img_cnt), (cap_cnt * condi_img_cnt))
            gt_render_imgs += [render_imgs[i] for i in indices]

        for cap in caption_list:
            for condi_img in condi_imgs:
                prompt_texts.append(cap)
                condi_img_paths.append(condi_img)
        uv_pos_paths += [uv_pos] * (cap_cnt * condi_img_cnt)
        uv_normal_paths += [uv_normal] * (cap_cnt * condi_img_cnt)
        uv_kd_paths += [uv_kd] * (cap_cnt * condi_img_cnt)

    return prompt_texts, uv_pos_paths, uv_normal_paths, condi_img_paths, uv_kd_paths, gt_render_imgs


def infer_img_uv_batch_helper(pipeline, generator, objs_dict, sample_key_pair_list, infer_batch_size, resolution=512, infer_sd=False):
    ...
    """
    # run inference
    prompt_texts, uv_pos_paths, uv_normal_paths, condi_img_paths, uv_kd_paths, gt_render_imgs = make_infer_data(objs_dict, sample_key_pair_list, sample_render_img=True)
