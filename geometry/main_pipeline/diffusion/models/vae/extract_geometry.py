...
from utils.mc.sparse_mc.sparse_marching_cubes import sparse_marching_cubes
...
def generate_dense_grid_points(
    ...
    x = np.linspace(bbox_min[0], bbox_max[0], int(
        num_cells) + 1, dtype=np.float32)
    y = np.linspace(bbox_min[1], bbox_max[1], int(
        num_cells) + 1, dtype=np.float32)
    z = np.linspace(bbox_min[2], bbox_max[2], int(
        num_cells) + 1, dtype=np.float32)
...
def extract_geometry(vae,
                     ...
                     bounds: Union[Tuple[float], List[float],
                                   float] = (-1.05, -1.05, -1.05, 1.05, 1.05, 1.05),
                     octree_depth: int = 8,
    ...
    if method == 'sparse':
        ...
        # vertices, faces = sparse_marching_cubes(query_func, init_depth, final_depth, 0.0, surface_in, surface_out, surface_range_decay, bounds=(-1,1), verbose=True)
        vertices, faces = sparse_marching_cubes(query_func, init_depth,
                                                final_depth, 0.0, surface_in, surface_out, surface_range_decay,
                                                bounds=(-box_v, box_v), verbose=False, device=latents.device,
                                                flip_faces=True)
        ...
        grid_logits = torch.cat(batch_logits, dim=1).view(
            (batch_size, grid_size[0], grid_size[1], grid_size[2])).float().numpy()
