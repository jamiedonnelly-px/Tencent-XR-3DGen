...
def test(args):
    ...
    gray_image_local_embedding = torch.cat([gray_image_local_embedding]*4, dim=1)
    gray_image_global_embedding = torch.cat([gray_image_global_embedding]*4, dim=1)
    ...
    mvimage_dir_list = [os.path.join(image_dir, x)
                        for x in os.listdir(image_dir)]
    mvimage_dir_list.sort()
    print(f"image_dir_list num: {len(mvimage_dir_list)}")

    num = 0
    for i, mvimage_dir in enumerate(mvimage_dir_list):
        if i >= num_samples:
            break
        mvimages_path_list = [os.path.join(
            mvimage_dir, x) for x in os.listdir(mvimage_dir)]
        if len(mvimages_path_list) < 1:
            print(mvimage_dir)
            continue
        mvimages_path_list.sort()

        # save image
        image_name = os.path.basename(mvimage_dir)
        ...
        # test diffusion
        try:
            images_mask_list = process_image_path_list(
                mvimages_path_list, bg_color=127, wh_ratio=0.9, rmbg_type="1.4")
            images_list = [x[0] for x in images_mask_list]
            for i in range(4 - len(images_list)):
                images_list.append(Image.fromarray(
                    np.zeros((512, 512, 3), dtype=np.uint8)))
        except:
            continue
        for i, image in enumerate(images_list):
            isave_path = os.path.join(mesh_name, f"{i}.png")
            image.save(isave_path)


        image_save_path = os.path.join(mesh_name, os.path.basename(mvimage_dir))
        os.system(f"cp -r \"{mvimage_dir}\" \"{image_save_path}\"")

        image_latents_local_dino = get_dino_feature(images_list)
        image_latents_local_dino = image_latents_local_dino.view(1, -1, 1024)
        image_latents_local_dino = image_latents_local_dino.to(
            dtype=weight_dtype).contiguous()

        image_latents_global_clip = get_clip_feature(images_list)
        image_latents_global_clip = image_latents_global_clip.view(1, -1)
        ...
        generator.to("cpu")
        classname = "test_images"
        objname = os.path.basename(image_dir)
        torch.cuda.empty_cache()
        ...
        try:
            ...
            print(f"extract_geometry failed: {image_dir}")
